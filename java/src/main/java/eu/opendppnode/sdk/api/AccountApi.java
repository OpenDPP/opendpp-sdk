...
import eu.opendppnode.sdk.model.WhoamiResponse;
...
public class AccountApi {
   ...
   * Returns a compact, integration-focused view of the calling credential: the workspace, the principal&#39;s role and resolved permissions, whether the session is an API key, the operator the key is scoped to (&#x60;null&#x60; &#x3D; workspace-wide), and active-passport usage against the tier quota. Use it to verify a key works, discover the effective permission set, and surface remaining quota.  Profile, localization and billing details are deliberately not exposed here — this is the integration view of the credential.  **Permission:** none beyond a valid tenant-scoped session — any API key can call it. Platform-admin sessions are rejected with &#x60;403&#x60; (they are not tenant-scoped).  **Rate limit:** your plan&#39;s per-key budget applies — **Growth** 120/min, **Scale** 600/min, **Enterprise** unlimited — with a ceiling of 3x that rate across all of the workspace&#39;s keys. The per-IP ceiling is not the binding limit for authenticated calls. Standard &#x60;x-ratelimit-*&#x60; headers; **429** carries &#x60;Retry-After&#x60;.
   * @return WhoamiResponse
   * @throws ApiException if fails to make API call
   */
  public WhoamiResponse whoami() throws ApiException {
    ApiResponse<WhoamiResponse> localVarResponse = whoamiWithHttpInfo();
   ...
   * Identity of the authenticated key / session
   * Returns a compact, integration-focused view of the calling credential: the workspace, the principal&#39;s role and resolved permissions, whether the session is an API key, the operator the key is scoped to (&#x60;null&#x60; &#x3D; workspace-wide), and active-passport usage against the tier quota. Use it to verify a key works, discover the effective permission set, and surface remaining quota.  Profile, localization and billing details are deliberately not exposed here — this is the integration view of the credential.  **Permission:** none beyond a valid tenant-scoped session — any API key can call it. Platform-admin sessions are rejected with &#x60;403&#x60; (they are not tenant-scoped).  **Rate limit:** your plan&#39;s per-key budget applies — **Growth** 120/min, **Scale** 600/min, **Enterprise** unlimited — with a ceiling of 3x that rate across all of the workspace&#39;s keys. The per-IP ceiling is not the binding limit for authenticated calls. Standard &#x60;x-ratelimit-*&#x60; headers; **429** carries &#x60;Retry-After&#x60;.
   * @return ApiResponse&lt;WhoamiResponse&gt;
   * @throws ApiException if fails to make API call
   */
  public ApiResponse<WhoamiResponse> whoamiWithHttpInfo() throws ApiException {
    ...
    try {
      ...
      try {
        ...
        if (localVarResponse.body() == null) {
          return new ApiResponse<WhoamiResponse>(
        ...
        return new ApiResponse<WhoamiResponse>(
            localVarResponse.statusCode(),
            localVarResponse.headers().map(),
            responseBody.isBlank()? null: memberVarObjectMapper.readValue(responseBody, new TypeReference<WhoamiResponse>() {})
        );
      } finally {
      }
    } catch (IOException e) {
