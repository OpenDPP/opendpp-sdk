...
class QRCodesApi:
    ...
    def get_battery_unit_qr_code(
        ...
        :type hri: bool
        :param x_dimension_mm: Width of ONE module in millimetres, chosen for the scanning environment the printed carrier will live in (EN 18220 5.5.2). When given, the SVG root is sized in mm — `width`/`height` become `<modules x xDimensionMm>mm`, with the `viewBox` preserving the geometry — and `size` is ignored. Range 0.396–2 mm inclusive. SVG only, and not combinable with `hri` (the HRI label is laid out in pixels, which would discard the millimetre root).
        :type x_dimension_mm: float
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
    ...
    def _get_battery_unit_qr_code_serialize(
        ...
        if x_dimension_mm is not None:
            
            _query_params.append(('xDimensionMm', x_dimension_mm))
