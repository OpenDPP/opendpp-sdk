...
class SealVerifyResponse(BaseModel):
    ...
    success: StrictBool
    verified: StrictBool
    message: Optional[StrictStr] = Field(default=None, description="Present only on the two policy failures: unregistered public key, or a declared operator not bound to the signing tenant.")
    ...
    def message_validate_enum(cls, value):
        ...
        if value not in set(['Cryptographic verification failed: The public key used to seal this passport is not registered to any authorized economic operator tenant on this node.', 'Cryptographic verification failed: The economic operator declared in this passport is not a registered operator bound to the signing tenant.']):
            raise ValueError("must be one of enum values ('Cryptographic verification failed: The public key used to seal this passport is not registered to any authorized economic operator tenant on this node.', 'Cryptographic verification failed: The economic operator declared in this passport is not a registered operator bound to the signing tenant.')")
        return value
