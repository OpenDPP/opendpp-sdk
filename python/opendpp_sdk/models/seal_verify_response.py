...
class SealVerifyResponse(BaseModel):
    ...
    success: StrictBool
    verified: StrictBool
    message: Optional[StrictStr] = Field(default=None, description="Present only on the two policy declines, each prefixed `Verification declined:` — an unregistered public key, or a declared operator not bound to the signing tenant. In both cases the seal was NOT cryptographically evaluated (this node verifies only seals issued by its own tenants). The wording is human-readable prose, not a stable contract value — match on `verified`, never on this string (deliberately no `enum` since 1.15.0).")
    certificate: Optional[SealCertificateReport] = None
    timestamp: Optional[SealTimestampReport] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "verified", "message", "certificate", "timestamp"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SealVerifyResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SealVerifyResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "certificate": SealCertificateReport.from_dict(obj["certificate"]) if obj.get("certificate") is not None else None,
            "timestamp": SealTimestampReport.from_dict(obj["timestamp"]) if obj.get("timestamp") is not None else None
        })
