...
class BatteryUnitRestrictedDataNotice(BaseModel):
    """
    ...
    how_to_request: StrictStr = Field(description="Relative URL `/request-access?unit={unitId}` where a legitimate-interest grant can be requested.", alias="howToRequest")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["reason", "reference", "description", "howToRequest"]

    @field_validator('reason')
    def reason_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LEGITIMATE_INTEREST_REQUIRED']):
            raise ValueError("must be one of enum values ('LEGITIMATE_INTEREST_REQUIRED')")
        return value

    @field_validator('reference')
    def reference_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Regulation (EU) 2023/1542, Annex XIII(2)-(4)']):
            raise ValueError("must be one of enum values ('Regulation (EU) 2023/1542, Annex XIII(2)-(4)')")
        return value

    @field_validator('description')
    def description_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Per-unit dynamic data (state of health, cycle counts, negative events, temperature) is accessible only to persons with a legitimate interest and to authorities.']):
            raise ValueError("must be one of enum values ('Per-unit dynamic data (state of health, cycle counts, negative events, temperature) is accessible only to persons with a legitimate interest and to authorities.')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitRestrictedDataNotice from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitRestrictedDataNotice from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "howToRequest": obj.get("howToRequest")
        })
