...
class RegisterOperatorRequest(BaseModel):
    ...
    """ # noqa: E501
    name: StrictStr = Field(description="Legal/display name. Ignored if your workspace already has an operator with this `regId` (the existing record is returned instead).")
    reg_id: StrictStr = Field(description="The identifier issued under `regIdScheme`: an EU VAT identification number (e.g. `DE811907980`), a D-U-N-S number (`150483782`), an LEI (`529900T8BM49AURSDO55`) or a GS1 GLN (`4012345000009`). Checked against the scheme's shape; fabricated `EORI-MOCK…` ids are rejected. Unique within your workspace; immutable once declared.", alias="regId")
    reg_id_scheme: StrictStr = Field(description="Required: the EN 18219 clause 6 scheme `regId` is issued under, matched case-insensitively (uppercased server-side). Any other value — an EORI included — is rejected with `400`. Ignored when binding to an existing operator.", alias="regIdScheme")
    eori: Optional[StrictStr] = Field(default=None, description="Optional EU EORI (customs identifier), `^[A-Z]{2}[A-Za-z0-9]{1,15}$` after whitespace is stripped and letters upper-cased. Not the EN 18219 identifier — it has no ISO/IEC 6523 ICD — but what customs and the EU registry know the operator by. Omit or `null` for none.")
    role: Optional[StrictStr] = Field(default='MANUFACTURER', description="Supply-chain role, free text — e.g. `MANUFACTURER`, `IMPORTER`, `RETAILER`. Defaults to `MANUFACTURER`. Ignored when binding to an existing operator.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["name", "regId", "regIdScheme", "eori", "role"]

    @field_validator('reg_id_scheme')
    def reg_id_scheme_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['VAT', 'DUNS', 'LEI', 'GLN']):
            raise ValueError("must be one of enum values ('VAT', 'DUNS', 'LEI', 'GLN')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RegisterOperatorRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # set to None if eori (nullable) is None
        # and model_fields_set contains the field
        if self.eori is None and "eori" in self.model_fields_set:
            _dict['eori'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RegisterOperatorRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "regId": obj.get("regId"),
            "regIdScheme": obj.get("regIdScheme"),
            "eori": obj.get("eori"),
            "role": obj.get("role") if obj.get("role") is not None else 'MANUFACTURER'
        })
