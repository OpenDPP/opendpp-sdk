...
class WebhookEnvelope(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Stable delivery id; mirrors the `X-OpenDPP-Delivery` header; constant across retries.")
    type: StrictStr = Field(description="Concrete event type (also in the `X-OpenDPP-Event` header). Never the `*` filter value.")
    created: datetime = Field(description="Event creation/enqueue time (stable across retries).")
    data: PublicPassportJsonLd = Field(description="The public (redacted) JSON-LD passport document — the same shape resolvers return.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "created", "data"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['passport.ingested', 'passport.updated', 'passport.sealed', 'passport.recalled', 'passport.status_updated']):
            raise ValueError("must be one of enum values ('passport.ingested', 'passport.updated', 'passport.sealed', 'passport.recalled', 'passport.status_updated')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "data": PublicPassportJsonLd.from_dict(obj["data"]) if obj.get("data") is not None else None
        })
