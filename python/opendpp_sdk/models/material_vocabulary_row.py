...
class MaterialVocabularyRow(BaseModel):
    """
    One entry of the platform-curated material vocabulary. Entries are unique per (`kind`, `name`).
    """ # noqa: E501
    id: StrictStr
    name: StrictStr = Field(description="Canonical display name, e.g. \"Organic Cotton\" or \"Lithium Iron Phosphate (LFP)\".")
    kind: StrictStr = Field(description="Vocabulary kind. `crm` = critical raw material.")
    cas_number: Optional[StrictStr] = Field(description="Optional CAS registry number (chemicals/substances); null when not applicable.", alias="casNumber")
    description: Optional[StrictStr] = Field(description="Optional short note shown in the picker; null when unset.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "name", "kind", "casNumber", "description"]

    @field_validator('kind')
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['material', 'fiber', 'chemistry', 'substance', 'hazard', 'crm']):
            raise ValueError("must be one of enum values ('material', 'fiber', 'chemistry', 'substance', 'hazard', 'crm')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MaterialVocabularyRow from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # set to None if cas_number (nullable) is None
        # and model_fields_set contains the field
        if self.cas_number is None and "cas_number" in self.model_fields_set:
            _dict['casNumber'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MaterialVocabularyRow from a dict"""
        if obj is None:
            return None
