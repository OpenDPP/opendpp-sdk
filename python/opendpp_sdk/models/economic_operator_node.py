...
class EconomicOperatorNode(BaseModel):
    """
    ...
    id: StrictStr
    name: StrictStr
    reg_id: StrictStr = Field(description="The identifier issued under `regIdScheme` — e.g. `LT000000000001`, an EU VAT identification number. Unique within the operator's workspace.", alias="regId")
    reg_id_scheme: StrictStr = Field(description="The EN 18219 clause 6 scheme `regId` is issued under — `VAT`, `DUNS`, `LEI` or `GLN`, each with an ISO/IEC 6523 ICD — so the header's `economicOperatorId` reads as `ICD:identifier`. `UNDECLARED` only on an operator that predates the declaration; its `economicOperatorId` is then the identifier as registered.", alias="regIdScheme")
    role: Optional[StrictStr] = Field(default=None, description="Operator role in the supply chain, e.g. `MANUFACTURER`, `IMPORTER`, `RETAILER`. Always present in detail/resolution responses; absent from `GET /api/v1/passports` list items.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@type", "id", "name", "regId", "regIdScheme", "role"]

    @field_validator('at_type')
    def at_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['EconomicOperator']):
            raise ValueError("must be one of enum values ('EconomicOperator')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of EconomicOperatorNode from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of EconomicOperatorNode from a dict"""
        if obj is None:
            return None
