...
REVOKEGRANT403RESPONSE_ANY_OF_SCHEMAS = ["Error", "GrantRouteError"]

class RevokeGrant403Response(BaseModel):
    """
    RevokeGrant403Response
    """

    # data type: GrantRouteError
    anyof_schema_1_validator: Optional[GrantRouteError] = None
    # data type: Error
    anyof_schema_2_validator: Optional[Error] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, GrantRouteError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "GrantRouteError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = RevokeGrant403Response.model_construct()
        error_messages = []
        # validate data type: GrantRouteError
        if not isinstance(v, GrantRouteError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GrantRouteError`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in RevokeGrant403Response with anyOf schemas: Error, GrantRouteError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[GrantRouteError] = None
        try:
            instance.actual_instance = GrantRouteError.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into RevokeGrant403Response with anyOf schemas: Error, GrantRouteError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, GrantRouteError]]:
