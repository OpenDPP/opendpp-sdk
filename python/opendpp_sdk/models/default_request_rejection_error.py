...
class DefaultRequestRejectionError(BaseModel):
    """
    ...
    code: Optional[StrictStr] = Field(default=None, description="Machine-readable error code, e.g. `FST_ERR_VALIDATION` (an envelope/schema violation) or `FST_ERR_CTP_INVALID_JSON_BODY` (a syntactically malformed JSON body); may be absent.")
    error: StrictStr
    message: StrictStr
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["statusCode", "code", "error", "message"]
    ...
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Bad Request']):
            raise ValueError("must be one of enum values ('Bad Request')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DefaultRequestRejectionError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DefaultRequestRejectionError from a dict"""
        if obj is None:
            return None
