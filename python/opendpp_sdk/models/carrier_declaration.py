...
class CarrierDeclaration(BaseModel):
    """
    ...
    """ # noqa: E501
    symbology: StrictStr = Field(description="EN 18220 Clause 6 carrier technology.")
    placement: StrictStr = Field(description="EN 18220 §5.4.2–5.4.6 marking or embedding method: on the product, on its packaging, as a label, in the accompanying document, or embedded in the product.")
    x_dimension_mm: Optional[Union[Annotated[float, Field(le=2, strict=True, ge=0.396)], Annotated[int, Field(le=2, strict=True, ge=1)]]] = Field(default=None, description="Module width in millimetres the symbol is printed at (§5.5.2; printed 2D symbols only). The same bounds `GET /api/v1/passports/{id}/qr?xDimensionMm=` accepts.", alias="xDimensionMm")
    error_correction: Optional[StrictStr] = Field(default=None, description="ISO/IEC 18004 error-correction level; QR Code only. All four levels are accepted because this records what the operator printed, not what this node draws: the QR endpoint's `ecl` parameter offers M, Q and H, so an `L` declaration describes a carrier the node would not have rendered.", alias="errorCorrection")
    target_environment: Optional[Annotated[str, Field(strict=True, max_length=160)]] = Field(default=None, description="The scanning environment the carrier was sized and graded for (§5.5.2, §5.6.1).", alias="targetEnvironment")
    print_quality_grade: Optional[Annotated[str, Field(strict=True, max_length=60)]] = Field(default=None, description="Print-quality grade and the method it was assessed by (§5.6.2). Free text, because the clause admits two methods whose grades take incompatible forms: a printed 2D symbol is graded under ISO/IEC 15415:2024 as overall grade / measuring aperture as a percentage of the X dimension / peak illumination wavelength in nm (e.g. `1.5/08/660`), while a direct part mark is graded under ISO/IEC 29158 as a letter (e.g. `DPM grade B`). State the method alongside the grade.", alias="printQualityGrade")
    durability_assessment: Optional[Annotated[str, Field(strict=True, max_length=500)]] = Field(default=None, description="The operator's durability assessment for a label or a reused product (§5.4.4, §4.3.1).", alias="durabilityAssessment")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["symbology", "placement", "xDimensionMm", "errorCorrection", "targetEnvironment", "printQualityGrade", "durabilityAssessment"]

    @field_validator('symbology')
    def symbology_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['QR_CODE', 'DATA_MATRIX', 'NFC', 'HF_RFID', 'RAIN_RFID']):
            raise ValueError("must be one of enum values ('QR_CODE', 'DATA_MATRIX', 'NFC', 'HF_RFID', 'RAIN_RFID')")
        return value

    @field_validator('placement')
    def placement_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PRODUCT', 'PACKAGING', 'LABEL', 'DOCUMENT', 'EMBEDDED']):
            raise ValueError("must be one of enum values ('PRODUCT', 'PACKAGING', 'LABEL', 'DOCUMENT', 'EMBEDDED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CarrierDeclaration from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CarrierDeclaration from a dict"""
        if obj is None:
            return None
