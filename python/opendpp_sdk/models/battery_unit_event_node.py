...
class BatteryUnitEventNode(BaseModel):
    """
    ...
    state_of_health: Optional[Union[StrictFloat, StrictInt]] = Field(description="Percent, 0-100.", alias="stateOfHealth")
    cycle_count: Optional[StrictInt] = Field(alias="cycleCount")
    remaining_capacity_ah: Optional[Union[StrictFloat, StrictInt]] = Field(alias="remainingCapacityAh")
    temperature_c: Optional[Union[StrictFloat, StrictInt]] = Field(alias="temperatureC")
    payload: Optional[Any]
    recorded_at: datetime = Field(description="When the measurement was taken (client-supplied at ingestion).", alias="recordedAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@type", "eventType", "stateOfHealth", "cycleCount", "remainingCapacityAh", "temperatureC", "payload", "recordedAt"]

    @field_validator('at_type')
    def at_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BatteryUnitEvent']):
            raise ValueError("must be one of enum values ('BatteryUnitEvent')")
        return value

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SOH_MEASUREMENT', 'CHARGE_CYCLE', 'STATUS_CHANGE', 'NEGATIVE_EVENT', 'OTHER']):
            raise ValueError("must be one of enum values ('SOH_MEASUREMENT', 'CHARGE_CYCLE', 'STATUS_CHANGE', 'NEGATIVE_EVENT', 'OTHER')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitEventNode from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitEventNode from a dict"""
        if obj is None:
            return None
