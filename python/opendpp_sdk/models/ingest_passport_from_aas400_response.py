...
INGESTPASSPORTFROMAAS400RESPONSE_ANY_OF_SCHEMAS = ["AASESPRValidationFailure", "Error"]

class IngestPassportFromAas400Response(BaseModel):
    """
    IngestPassportFromAas400Response
    """

    # data type: Error
    anyof_schema_1_validator: Optional[Error] = None
    # data type: AASESPRValidationFailure
    anyof_schema_2_validator: Optional[AASESPRValidationFailure] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AASESPRValidationFailure, Error]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "AASESPRValidationFailure", "Error" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = IngestPassportFromAas400Response.model_construct()
        error_messages = []
        ...
        # validate data type: AASESPRValidationFailure
        if not isinstance(v, AASESPRValidationFailure):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AASESPRValidationFailure`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in IngestPassportFromAas400Response with anyOf schemas: AASESPRValidationFailure, Error. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[AASESPRValidationFailure] = None
        try:
            instance.actual_instance = AASESPRValidationFailure.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into IngestPassportFromAas400Response with anyOf schemas: AASESPRValidationFailure, Error. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AASESPRValidationFailure, Error]]:
