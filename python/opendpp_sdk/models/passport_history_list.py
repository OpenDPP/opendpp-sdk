...
class PassportHistoryList(BaseModel):
    """
    PassportHistoryList
    """ # noqa: E501
    success: StrictBool
    passport_id: StrictStr = Field(description="The passport UUID.", alias="passportId")
    product_id: StrictStr = Field(description="The caller-supplied product identifier.", alias="productId")
    current_version: Annotated[int, Field(strict=True, ge=1)] = Field(description="The live passport's version number: one more than the number of archived versions.", alias="currentVersion")
    page: StrictInt
    limit: StrictInt
    total: StrictInt = Field(description="Number of archived versions.")
    total_pages: StrictInt = Field(alias="totalPages")
    versions: List[PassportHistoryVersionSummary] = Field(description="Archived versions, newest first.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "passportId", "productId", "currentVersion", "page", "limit", "total", "totalPages", "versions"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportHistoryList from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportHistoryList from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "versions": [PassportHistoryVersionSummary.from_dict(_item) for _item in obj["versions"]] if obj.get("versions") is not None else None
        })
