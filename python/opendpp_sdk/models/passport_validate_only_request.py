...
class PassportValidateOnlyRequest(BaseModel):
    """
    A metadata payload to validate against its ESPR category rules without persisting anything.
    """ # noqa: E501
    product_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Product identifier (GTIN-14 / GRAI / SKU). Required and checked non-empty (whitespace-only → 400), but not otherwise used by the dry-run.", alias="productId")
    operator_id: Optional[StrictStr] = Field(default=None, description="Accepted by the body schema but IGNORED by the validate-only handlers.", alias="operatorId")
    metadata: PassportMetadataInput
    carrier: Optional[CarrierDeclaration] = Field(default=None, description="Optional. Checked against the EN 18220 vocabulary exactly as on create — an invalid declaration is a 400 here too, so a pre-flight check cannot pass a payload the save would refuse — and, when valid, advised on in `warnings` (a symbology this node does not draw, a granularity the product group's instrument does not expect). Nothing is persisted.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["productId", "operatorId", "metadata", "carrier"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyRequest from a dict"""
        if obj is None:
            return None
