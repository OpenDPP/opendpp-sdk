...
class PassportValidateOnlyResult(BaseModel):
    """
    200 envelope of the validate-only endpoints (only the declared keys are emitted).
    """ # noqa: E501
    success: StrictBool
    message: StrictStr
    category: StrictStr = Field(description="Echo of `metadata.category` (or \"unknown\" if absent).")
    errors: Annotated[List[ValidationErrorItem], Field(max_length=0)] = Field(description="Always an empty array on 200.")
    warnings: Optional[List[ValidationErrorItem]] = Field(default=None, description="Non-blocking findings (e.g. malformed supplementary objects like `circularityAttributes`). The key is OMITTED entirely when there are no warnings — it is never an empty array.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "category", "errors", "warnings"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyResult from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyResult from a dict"""
        if obj is None:
            return None
