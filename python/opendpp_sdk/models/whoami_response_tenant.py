...
class WhoamiResponseTenant(BaseModel):
    """
    WhoamiResponseTenant
    """ # noqa: E501
    id: StrictStr = Field(description="Workspace (tenant) id.")
    name: StrictStr = Field(description="Workspace company name.")
    subdomain: Optional[StrictStr] = Field(description="Workspace subdomain (`<subdomain>.opendpp-node.eu`), or null if none is assigned.")
    tier: StrictStr = Field(description="Subscription tier (e.g. `pilot`, `micro`, `starter`, `growth`, `scale`, `enterprise`).")
    subscription_status: Optional[StrictStr] = Field(description="Billing status string (e.g. `active`, `past_due`). When not `active`, write operations may return `402`. No amounts or processor identifiers are exposed.", alias="subscriptionStatus")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "name", "subdomain", "tier", "subscriptionStatus"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WhoamiResponseTenant from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WhoamiResponseTenant from a dict"""
        if obj is None:
            return None
