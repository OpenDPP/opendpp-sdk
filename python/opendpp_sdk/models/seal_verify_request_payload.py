...
class SealVerifyRequestPayload(BaseModel):
    """
    ...
    """ # noqa: E501
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="The sealed metadata object — the SHA-256 Merkle tree is rebuilt over its top-level properties (every leaf recomputed from the actual values; caller-supplied redacted-leaf hashes are not accepted). When the `metadata` key is entirely ABSENT, the whole `payload` object is treated as the metadata for the Merkle phase; a present-but-non-object value skips the Merkle phase (only the whole-payload fallback runs).")
    operator: Optional[SealVerifyRequestPayloadOperator] = None
    economic_operator: Optional[SealVerifyRequestPayloadEconomicOperator] = Field(default=None, alias="economicOperator")
    proof: Optional[SealVerifyRequestPayloadProof] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["metadata", "operator", "economicOperator", "proof"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SealVerifyRequestPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SealVerifyRequestPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "operator": SealVerifyRequestPayloadOperator.from_dict(obj["operator"]) if obj.get("operator") is not None else None,
            "economicOperator": SealVerifyRequestPayloadEconomicOperator.from_dict(obj["economicOperator"]) if obj.get("economicOperator") is not None else None,
            "proof": SealVerifyRequestPayloadProof.from_dict(obj["proof"]) if obj.get("proof") is not None else None
        })
