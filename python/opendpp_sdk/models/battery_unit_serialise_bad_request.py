...
BATTERYUNITSERIALISEBADREQUEST_ONE_OF_SCHEMAS = ["BatteryUnitSerialisationFailedError", "DefaultRequestRejectionError", "Error"]

class BatteryUnitSerialiseBadRequest(BaseModel):
    """
    ...
    # data type: BatteryUnitSerialisationFailedError
    oneof_schema_2_validator: Optional[BatteryUnitSerialisationFailedError] = None
    # data type: DefaultRequestRejectionError
    oneof_schema_3_validator: Optional[DefaultRequestRejectionError] = None
    actual_instance: Optional[Union[BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error]] = None
    one_of_schemas: Set[str] = { "BatteryUnitSerialisationFailedError", "DefaultRequestRejectionError", "Error" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BatteryUnitSerialiseBadRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Error):
            ...
            match += 1
        # validate data type: BatteryUnitSerialisationFailedError
        if not isinstance(v, BatteryUnitSerialisationFailedError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BatteryUnitSerialisationFailedError`")
        else:
            match += 1
        # validate data type: DefaultRequestRejectionError
        if not isinstance(v, DefaultRequestRejectionError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DefaultRequestRejectionError`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BatteryUnitSerialiseBadRequest with oneOf schemas: BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BatteryUnitSerialiseBadRequest with oneOf schemas: BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BatteryUnitSerialisationFailedError
        try:
            instance.actual_instance = BatteryUnitSerialisationFailedError.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DefaultRequestRejectionError
        try:
            instance.actual_instance = DefaultRequestRejectionError.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BatteryUnitSerialiseBadRequest with oneOf schemas: BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BatteryUnitSerialiseBadRequest with oneOf schemas: BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BatteryUnitSerialisationFailedError, DefaultRequestRejectionError, Error]]:
