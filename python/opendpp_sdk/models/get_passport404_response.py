...
GETPASSPORT404RESPONSE_ANY_OF_SCHEMAS = ["Error", "GetPassport404ResponseAnyOf"]

class GetPassport404Response(BaseModel):
    """
    GetPassport404Response
    """

    # data type: Error
    anyof_schema_1_validator: Optional[Error] = None
    # data type: GetPassport404ResponseAnyOf
    anyof_schema_2_validator: Optional[GetPassport404ResponseAnyOf] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, GetPassport404ResponseAnyOf]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "GetPassport404ResponseAnyOf" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetPassport404Response.model_construct()
        error_messages = []
        ...
        # validate data type: GetPassport404ResponseAnyOf
        if not isinstance(v, GetPassport404ResponseAnyOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GetPassport404ResponseAnyOf`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetPassport404Response with anyOf schemas: Error, GetPassport404ResponseAnyOf. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[GetPassport404ResponseAnyOf] = None
        try:
            instance.actual_instance = GetPassport404ResponseAnyOf.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetPassport404Response with anyOf schemas: Error, GetPassport404ResponseAnyOf. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, GetPassport404ResponseAnyOf]]:
