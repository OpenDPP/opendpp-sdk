...
class OperatorRow(BaseModel):
    ...
    reg_id: StrictStr = Field(description="Official registration id (EORI number, VAT id, DUNS, or national business-registry id). Unique within your workspace and immutable after registration.", alias="regId")
    reg_id_scheme: Optional[StrictStr] = Field(description="Which kind of registration id `regId` is. `null` = unspecified national/business id. When `EORI`, `regId` is guaranteed to satisfy the EORI syntax `^[A-Z]{2}[A-Za-z0-9]{1,15}$`.", alias="regIdScheme")
    ...
    __properties: ClassVar[List[str]] = ["id", "name", "regId", "regIdScheme", "role", "archivedAt", "createdAt"]
    ...
    def reg_id_scheme_validate_enum(cls, value):
        ...
        if value not in set(['EORI', 'VAT', 'DUNS', 'NATIONAL', 'OTHER']):
            raise ValueError("must be one of enum values ('EORI', 'VAT', 'DUNS', 'NATIONAL', 'OTHER')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # set to None if reg_id_scheme (nullable) is None
        # and model_fields_set contains the field
        if self.reg_id_scheme is None and "reg_id_scheme" in self.model_fields_set:
            _dict['regIdScheme'] = None
