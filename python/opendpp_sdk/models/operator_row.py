...
class OperatorRow(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Operator UUID.")
    name: StrictStr = Field(description="Legal/display name of the operator.")
    reg_id: StrictStr = Field(description="The identifier issued under `regIdScheme` — an EU VAT identification number, a D-U-N-S number, an LEI or a GS1 GLN. Unique within your workspace and immutable once declared.", alias="regId")
    reg_id_scheme: StrictStr = Field(description="The EN 18219 clause 6 scheme `regId` is issued under. Each has an ISO/IEC 6523 ICD — VAT `0223`, DUNS `0060`, LEI `0199`, GLN `0088` — which is how the EN 18223 `economicOperatorId` is written. `UNDECLARED` appears only on an operator that predates the declaration and whose identifier the migration could not classify: it cannot be sent, and such an operator is declared once via `PATCH`.", alias="regIdScheme")
    eori: Optional[StrictStr] = Field(description="The operator's EU EORI (customs identifier), normalised, or `null`. Carried beside `regId`: it has no ISO/IEC 6523 ICD and is not what the EN 18223 header names.")
    role: StrictStr = Field(description="Supply-chain role, free text — e.g. `\"MANUFACTURER\"`, `\"IMPORTER\"`, `\"RETAILER\"`. Defaults to `\"MANUFACTURER\"` at registration.")
    archived_at: Optional[datetime] = Field(description="Soft-delete / cessation-of-trading marker. Non-null = the operator is archived (its passports are retained and still publicly resolvable).", alias="archivedAt")
    created_at: datetime = Field(alias="createdAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "name", "regId", "regIdScheme", "eori", "role", "archivedAt", "createdAt"]

    @field_validator('reg_id_scheme')
    def reg_id_scheme_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['VAT', 'DUNS', 'LEI', 'GLN', 'UNDECLARED']):
            raise ValueError("must be one of enum values ('VAT', 'DUNS', 'LEI', 'GLN', 'UNDECLARED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OperatorRow from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OperatorRow from a dict"""
        if obj is None:
            return None
