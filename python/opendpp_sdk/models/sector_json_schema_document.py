...
class SectorJsonSchemaDocument(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["$schema", "title", "type", "required", "properties"]

    @field_validator('var_schema')
    def var_schema_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['http://json-schema.org/draft-07/schema#']):
            raise ValueError("must be one of enum values ('http://json-schema.org/draft-07/schema#')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SectorJsonSchemaDocument from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SectorJsonSchemaDocument from a dict"""
        if obj is None:
            return None
