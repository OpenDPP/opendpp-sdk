...
class BatteryUnitEventListResponse(BaseModel):
    """
    One page of a battery unit's append-only dynamic-data history, newest first.
    """ # noqa: E501
    success: StrictBool
    count: Annotated[int, Field(le=500, strict=True, ge=0)] = Field(description="Equals `events.length`; never exceeds the page `limit`.")
    serial_number: StrictStr = Field(description="The unit's physical serial (GS1 AI-21 value).", alias="serialNumber")
    next_cursor: Optional[StrictStr] = Field(description="Opaque cursor for the next (older) page — pass it as the `cursor` query parameter. `null` when this page ends the history.", alias="nextCursor")
    events: Annotated[List[BatteryUnitEventRow], Field(max_length=500)] = Field(description="Newest first by `recordedAt` (ties broken by `id`), at most one page (`limit`) per response.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "count", "serialNumber", "nextCursor", "events"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitEventListResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitEventListResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "events": [BatteryUnitEventRow.from_dict(_item) for _item in obj["events"]] if obj.get("events") is not None else None
        })
