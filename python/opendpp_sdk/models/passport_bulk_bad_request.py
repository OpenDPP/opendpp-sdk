...
PASSPORTBULKBADREQUEST_ONE_OF_SCHEMAS = ["DefaultRequestRejectionError", "PassportBulkFailure"]

class PassportBulkBadRequest(BaseModel):
    """
    ...
    oneof_schema_2_validator: Optional[DefaultRequestRejectionError] = None
    actual_instance: Optional[Union[DefaultRequestRejectionError, PassportBulkFailure]] = None
    one_of_schemas: Set[str] = { "DefaultRequestRejectionError", "PassportBulkFailure" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PassportBulkBadRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PassportBulkBadRequest with oneOf schemas: DefaultRequestRejectionError, PassportBulkFailure. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PassportBulkBadRequest with oneOf schemas: DefaultRequestRejectionError, PassportBulkFailure. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PassportBulkBadRequest with oneOf schemas: DefaultRequestRejectionError, PassportBulkFailure. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PassportBulkBadRequest with oneOf schemas: DefaultRequestRejectionError, PassportBulkFailure. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DefaultRequestRejectionError, PassportBulkFailure]]:
