...
REGISTEROPERATOR400RESPONSE_ANY_OF_SCHEMAS = ["Error", "OperatorMinimalError"]

class RegisterOperator400Response(BaseModel):
    """
    RegisterOperator400Response
    """

    # data type: OperatorMinimalError
    anyof_schema_1_validator: Optional[OperatorMinimalError] = None
    # data type: Error
    anyof_schema_2_validator: Optional[Error] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, OperatorMinimalError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "OperatorMinimalError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = RegisterOperator400Response.model_construct()
        error_messages = []
        # validate data type: OperatorMinimalError
        if not isinstance(v, OperatorMinimalError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OperatorMinimalError`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in RegisterOperator400Response with anyOf schemas: Error, OperatorMinimalError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[OperatorMinimalError] = None
        try:
            instance.actual_instance = OperatorMinimalError.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into RegisterOperator400Response with anyOf schemas: Error, OperatorMinimalError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, OperatorMinimalError]]:
