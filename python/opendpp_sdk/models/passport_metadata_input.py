...
class PassportMetadataInput(BaseModel):
    """
    ...
    """ # noqa: E501
    category: Optional[StrictStr] = Field(default=None, description="ESPR product category; selects the validation rules. Required whenever validation runs (i.e. always, except `draft: true` single ingestion).")
    origin_country: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="ISO 3166-1 alpha-2 country code (validated against the full 249-code set).", alias="originCountry")
    commodity_code: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Optional HS / TARIC commodity code (4–10 digits). Validated for every category when present (a malformed code is a 400). Not category-mandated, but required to project the passport into the EU DPP registry pointer (ESPR Art. 13).", alias="commodityCode")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["category", "originCountry", "commodityCode"]
    ...
    def category_validate_enum(cls, value):
        ...
        if value not in set(['textiles', 'batteries', 'electronics', 'cosmetics', 'toys', 'iron-steel', 'aluminium', 'chemicals', 'construction']):
            raise ValueError("must be one of enum values ('textiles', 'batteries', 'electronics', 'cosmetics', 'toys', 'iron-steel', 'aluminium', 'chemicals', 'construction')")
        return value

    @field_validator('origin_country')
    def origin_country_validate_regular_expression(cls, value):
    ...
    def commodity_code_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^[0-9]{4,10}$", value):
            raise ValueError(r"must validate the regular expression /^[0-9]{4,10}$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportMetadataInput from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportMetadataInput from a dict"""
        if obj is None:
            return None
