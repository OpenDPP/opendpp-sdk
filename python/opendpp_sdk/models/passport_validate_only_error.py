...
class PassportValidateOnlyError(BaseModel):
    """
    ...
    error: StrictStr
    message: StrictStr
    category: Optional[StrictStr] = Field(default=None, description="`metadata.category` echo, or \"unknown\" for structural failures.")
    errors: Optional[List[ValidationErrorItem]] = None
    warnings: Optional[List[ValidationErrorItem]] = Field(default=None, description="Omitted when there are no warnings.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "error", "message", "category", "errors", "warnings"]

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Bad Request', 'Validation Failed']):
            raise ValueError("must be one of enum values ('Bad Request', 'Validation Failed')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportValidateOnlyError from a dict"""
        if obj is None:
            return None
