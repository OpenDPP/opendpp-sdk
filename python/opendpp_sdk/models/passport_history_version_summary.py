...
class PassportHistoryVersionSummary(BaseModel):
    """
    PassportHistoryVersionSummary
    """ # noqa: E501
    version: Annotated[int, Field(strict=True, ge=1)] = Field(description="Archived version number; 1 is the state the first change replaced.")
    valid_until: datetime = Field(description="The instant this version stopped being current — when the change that replaced it was recorded.", alias="validUntil")
    recorded_at: datetime = Field(description="When the snapshot was archived (the same instant as `validUntil`).", alias="recordedAt")
    changed_by: StrictStr = Field(description="Who recorded the change that replaced this version — an API-key or user label.", alias="changedBy")
    change_reason: Optional[StrictStr] = Field(description="The caller's `changeReason` on the write, or the lifecycle transition (`Status changed: ACTIVE → RECALLED`).", alias="changeReason")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["version", "validUntil", "recordedAt", "changedBy", "changeReason"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportHistoryVersionSummary from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportHistoryVersionSummary from a dict"""
        if obj is None:
            return None
