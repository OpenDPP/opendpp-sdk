...
class UntpEventCredentialSubject(BaseModel):
    """
    ...
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="EPC identifier of the subject (e.g. `urn:epc:id:sgtin:0950110153.0003.SN-2026-000123`). When `epcList` is not supplied as an array, the stored EPC list defaults to `[id]` (or `[]` if absent).")
    event_type: StrictStr = Field(description="EPCIS 2.0 event type (server-side enum).", alias="eventType")
    action: Optional[StrictStr] = Field(default=None, description="EPCIS action (server-side enum). Optional — but MUST be absent (or null) on `TransformationEvent`, otherwise 400 `Schema Validation Error`. An unknown value is rejected at the persistence layer (500).")
    biz_step: Optional[StrictStr] = Field(default='urn:epcglobal:cbv:bizstep:receiving', description="CBV business step URI. Defaults to `urn:epcglobal:cbv:bizstep:receiving`.", alias="bizStep")
    disposition: Optional[StrictStr] = Field(default='urn:epcglobal:cbv:disp:in_progress', description="CBV disposition URI. Defaults to `urn:epcglobal:cbv:disp:in_progress`.")
    read_point: Optional[StrictStr] = Field(default=None, description="Where the event was observed (e.g. `geo:41.1496,-8.6109`). When absent and `originLocation` is present, defaults to `geo:<latitude>,<longitude>`.", alias="readPoint")
    biz_location: Optional[StrictStr] = Field(default=None, description="Business location (SGLN URI, DID, or free identifier). When absent, defaults to `responsibleOperatorDid`.", alias="bizLocation")
    event_time: Optional[datetime] = Field(default=None, description="When the event occurred (anything `new Date()` parses). Defaults to the credential `issuanceDate`, else the server clock.", alias="eventTime")
    epc_list: Optional[List[StrictStr]] = Field(default=None, description="EPC URIs observed by the event. A non-array value is replaced with `[credentialSubject.id]` (or `[]`).", alias="epcList")
    parent_epc: Optional[StrictStr] = Field(default=None, description="Parent EPC for AggregationEvent.", alias="parentEpc")
    child_epcs: Optional[List[StrictStr]] = Field(default=None, description="Child EPCs for AggregationEvent (stored as JSON verbatim).", alias="childEpcs")
    input_epc_list: Optional[List[StrictStr]] = Field(default=None, description="Input EPCs for TransformationEvent (stored as JSON verbatim).", alias="inputEpcList")
    output_epc_list: Optional[List[StrictStr]] = Field(default=None, description="Output EPCs for TransformationEvent (stored as JSON verbatim).", alias="outputEpcList")
    origin_location: Optional[UntpEventCredentialSubjectOriginLocation] = Field(default=None, alias="originLocation")
    responsible_operator_did: Optional[StrictStr] = Field(default=None, description="DID of the responsible economic operator. Fallback for `bizLocation`, and (only when `issuer` is absent) for the operator-scope check on operator-scoped API keys.", alias="responsibleOperatorDid")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "eventType", "action", "bizStep", "disposition", "readPoint", "bizLocation", "eventTime", "epcList", "parentEpc", "childEpcs", "inputEpcList", "outputEpcList", "originLocation", "responsibleOperatorDid"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ObjectEvent', 'AggregationEvent', 'TransformationEvent', 'AssociationEvent']):
            raise ValueError("must be one of enum values ('ObjectEvent', 'AggregationEvent', 'TransformationEvent', 'AssociationEvent')")
        return value
    ...
    def action_validate_enum(cls, value):
        ...
        if value not in set(['ADD', 'OBSERVE', 'DELETE']):
            raise ValueError("must be one of enum values ('ADD', 'OBSERVE', 'DELETE')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UntpEventCredentialSubject from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of origin_location
        if self.origin_location:
            _dict['originLocation'] = self.origin_location.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UntpEventCredentialSubject from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "bizStep": obj.get("bizStep") if obj.get("bizStep") is not None else 'urn:epcglobal:cbv:bizstep:receiving',
            "disposition": obj.get("disposition") if obj.get("disposition") is not None else 'urn:epcglobal:cbv:disp:in_progress',
            "readPoint": obj.get("readPoint"),
            "bizLocation": obj.get("bizLocation"),
            "eventTime": obj.get("eventTime"),
            "epcList": obj.get("epcList"),
            "parentEpc": obj.get("parentEpc"),
            "childEpcs": obj.get("childEpcs"),
            "inputEpcList": obj.get("inputEpcList"),
            "outputEpcList": obj.get("outputEpcList"),
            "originLocation": UntpEventCredentialSubjectOriginLocation.from_dict(obj["originLocation"]) if obj.get("originLocation") is not None else None,
            "responsibleOperatorDid": obj.get("responsibleOperatorDid")
        })
