...
CREATEPASSPORT400RESPONSE_ANY_OF_SCHEMAS = ["ESPRValidationFailure", "Error"]

class CreatePassport400Response(BaseModel):
    """
    CreatePassport400Response
    """

    # data type: ESPRValidationFailure
    anyof_schema_1_validator: Optional[ESPRValidationFailure] = None
    # data type: Error
    anyof_schema_2_validator: Optional[Error] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ESPRValidationFailure, Error]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ESPRValidationFailure", "Error" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CreatePassport400Response.model_construct()
        error_messages = []
        # validate data type: ESPRValidationFailure
        if not isinstance(v, ESPRValidationFailure):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ESPRValidationFailure`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CreatePassport400Response with anyOf schemas: ESPRValidationFailure, Error. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ESPRValidationFailure] = None
        try:
            instance.actual_instance = ESPRValidationFailure.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CreatePassport400Response with anyOf schemas: ESPRValidationFailure, Error. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ESPRValidationFailure, Error]]:
