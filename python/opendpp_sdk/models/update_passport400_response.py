...
UPDATEPASSPORT400RESPONSE_ANY_OF_SCHEMAS = ["Error", "PassportUpdateValidationError"]

class UpdatePassport400Response(BaseModel):
    """
    UpdatePassport400Response
    """

    # data type: Error
    anyof_schema_1_validator: Optional[Error] = None
    # data type: PassportUpdateValidationError
    anyof_schema_2_validator: Optional[PassportUpdateValidationError] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, PassportUpdateValidationError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "PassportUpdateValidationError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = UpdatePassport400Response.model_construct()
        error_messages = []
        ...
        # validate data type: PassportUpdateValidationError
        if not isinstance(v, PassportUpdateValidationError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PassportUpdateValidationError`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in UpdatePassport400Response with anyOf schemas: Error, PassportUpdateValidationError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[PassportUpdateValidationError] = None
        try:
            instance.actual_instance = PassportUpdateValidationError.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UpdatePassport400Response with anyOf schemas: Error, PassportUpdateValidationError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, PassportUpdateValidationError]]:
