...
class PassportCreateValidationError(BaseModel):
    """
    ...
    error: StrictStr
    message: StrictStr
    errors: List[ValidationErrorItem] = Field(description="Blocking findings. Items produced by the category-validity check (`metadata.category` missing/unknown) carry no `friendlyMessage`.")
    warnings: Optional[List[ValidationErrorItem]] = Field(default=None, description="Omitted entirely when there are no warnings.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "error", "message", "errors", "warnings"]

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Validation Failed']):
            raise ValueError("must be one of enum values ('Validation Failed')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportCreateValidationError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportCreateValidationError from a dict"""
        if obj is None:
            return None
