...
class SealCertificateReport(BaseModel):
    """
    ...
    """ # noqa: E501
    subject: Optional[StrictStr] = Field(default=None, description="Leaf-certificate subject (multi-line RDN string as produced by Node's X509Certificate, e.g. `CN=OpenDPP Demo Eco Industries Seal`).")
    issuer: Optional[StrictStr] = Field(default=None, description="Leaf-certificate issuer RDN string.")
    valid_from: Optional[StrictStr] = Field(default=None, description="X.509 textual date, e.g. `Jan 10 00:00:00 2026 GMT` — NOT ISO 8601.", alias="validFrom")
    valid_to: Optional[StrictStr] = Field(default=None, description="X.509 textual date — NOT ISO 8601.", alias="validTo")
    chain_valid: StrictBool = Field(description="True only when every chain link signature-verifies, every certificate is within its validity window, AND the top of the chain is anchored to this node's seal CA (fingerprint match or signature under the CA key).", alias="chainValid")
    key_matches_proof: Optional[StrictBool] = Field(default=None, description="True when the leaf SPKI equals the supplied `publicKey` (whitespace-insensitive), or when no explicit public key was supplied (the leaf key was used).", alias="keyMatchesProof")
    error: Optional[StrictStr] = Field(default=None, description="Present only when the chain could not be parsed.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["subject", "issuer", "validFrom", "validTo", "chainValid", "keyMatchesProof", "error"]
    ...
    def error_validate_enum(cls, value):
        ...
        if value not in set(['Unparseable x5c certificate chain']):
            raise ValueError("must be one of enum values ('Unparseable x5c certificate chain')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SealCertificateReport from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SealCertificateReport from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "chainValid": obj.get("chainValid"),
            "keyMatchesProof": obj.get("keyMatchesProof"),
            "error": obj.get("error")
        })
