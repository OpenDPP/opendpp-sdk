...
class PublicPassportJsonLdExpanded(BaseModel):
    """
    ...
    digital_link_uri: StrictStr = Field(description="SKU/type-level GS1 Digital Link URI: `{origin}/{01|8003}/{productId}` — the bare primary key. Individual units carry their physical serial under AI 21 (`…/21/{serialNumber}`).", alias="digitalLinkUri")
    product_id_scheme: StrictStr = Field(description="The EN 18219 clause 5 scheme `productId` was declared under at issue: `GS1_DIGITAL_LINK` (scheme 1 — a GS1 key carried as a GS1 Digital Link) or `IDENTIFICATION_LINK` (scheme 2 — an EN IEC 61406-1 Identification Link under this node's domain for a non-GS1 identifier). Stated beside the identifier so a registry can keep it unique across identifier domains (EN 18219 4.1.2). Immutable once issued.", alias="productIdScheme")
    digital_product_passport_id: StrictStr = Field(description="The identifier of this PASSPORT instance (EN 18223 Table 1 `digitalProductPassportId`): this node's own `/passport/{id}` URL for the SKU/type passport. It identifies the passport, **not** the product — the product's identifier is `uniqueProductIdentifier` below, and the two are never the same value. `@id` and `digitalLinkUri` carry the product's link, so this attribute differs from both.", alias="digitalProductPassportId")
    ...
    granularity: StrictStr = Field(description="Granularity level of the unique product identifier — EN 18223 4.1.2.2's closed enumeration — as the passport declared it at issue: `model` for a GTIN-keyed or Identification-Link passport, `item` for a serialised GRAI. `batch` is in the enumeration and never issued by this node.")
    ...
    elements: List[En18223DataElement] = Field(description="The body's data elements, expanded (Annex A).")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@context", "@type", "@id", "id", "productId", "digitalLinkUri", "productIdScheme", "digitalProductPassportId", "uniqueProductIdentifier", "granularity", "dppSchemaVersion", "dppStatus", "lastUpdated", "economicOperatorId", "facilityId", "contentSpecificationIds", "digitalSeal", "signingPublicKey", "status", "archivedAt", "retentionUntil", "proof", "elements"]
    ...
    @field_validator('product_id_scheme')
    def product_id_scheme_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GS1_DIGITAL_LINK', 'IDENTIFICATION_LINK']):
            raise ValueError("must be one of enum values ('GS1_DIGITAL_LINK', 'IDENTIFICATION_LINK')")
        return value

    @field_validator('granularity')
    def granularity_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['model', 'batch', 'item']):
            raise ValueError("must be one of enum values ('model', 'batch', 'item')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PublicPassportJsonLdExpanded from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PublicPassportJsonLdExpanded from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "productIdScheme": obj.get("productIdScheme"),
            ...
            "elements": [En18223DataElement.from_dict(_item) for _item in obj["elements"]] if obj.get("elements") is not None else None
        })
