...
class PassportStatusUpdateResponse(BaseModel):
    """
    ...
    message: StrictStr = Field(description="`Passport status successfully updated to <STATUS>`.")
    status: StrictStr = Field(description="The passport's new lifecycle state.")
    passport: PublicPassportJsonLd
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "status", "passport"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTIVE', 'RECALLED', 'DECOMMISSIONED']):
            raise ValueError("must be one of enum values ('ACTIVE', 'RECALLED', 'DECOMMISSIONED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportStatusUpdateResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportStatusUpdateResponse from a dict"""
        if obj is None:
            return None
