...
class Error(BaseModel):
    ...
    """ # noqa: E501
    success: Optional[StrictBool] = Field(default=None, description="Always `false` when present. Omitted by public endpoints and some self-service endpoints.")
    error: StrictStr = Field(description="Short error title (usually the HTTP reason phrase).")
    message: StrictStr = Field(description="Human-readable explanation.")
    request_id: Optional[StrictStr] = Field(default=None, description="Correlation id for this request, also returned as the `X-Request-Id` response header on EVERY response. Present in generic (server-error / framework) bodies; quote it to support to correlate with server logs. Adopts a well-formed inbound `X-Request-Id` if you send one.", alias="requestId")
    code: Optional[StrictStr] = Field(default=None, description="Optional MACHINE-STABLE error code for the developer-facing write/ingest surface (passport / operator / unit / resolver / facility / events / webhooks) — branch on this instead of parsing `message`. Present on the errors it covers — the `code` enum below is the full set — and omitted otherwise.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "error", "message", "requestId", "code"]
    ...
    def code_validate_enum(cls, value):
        ...
        if value not in set(['OPERATOR_NOT_BOUND', 'OPERATOR_AMBIGUOUS', 'OPERATOR_SCOPE_FORBIDDEN', 'GTIN_CHECK_DIGIT_INVALID', 'GLN_CHECK_DIGIT_INVALID', 'COMPRESSED_DIGITAL_LINK', 'PASSPORT_DUPLICATE', 'PASSPORT_SEALED_IMMUTABLE', 'CATEGORY_IMMUTABLE', 'FACILITY_NOT_FOUND', 'FACILITY_DUPLICATE', 'WEBHOOK_NOT_FOUND', 'WEBHOOK_LIMIT_REACHED', 'WEBHOOK_URL_REJECTED']):
            raise ValueError("must be one of enum values ('OPERATOR_NOT_BOUND', 'OPERATOR_AMBIGUOUS', 'OPERATOR_SCOPE_FORBIDDEN', 'GTIN_CHECK_DIGIT_INVALID', 'GLN_CHECK_DIGIT_INVALID', 'COMPRESSED_DIGITAL_LINK', 'PASSPORT_DUPLICATE', 'PASSPORT_SEALED_IMMUTABLE', 'CATEGORY_IMMUTABLE', 'FACILITY_NOT_FOUND', 'FACILITY_DUPLICATE', 'WEBHOOK_NOT_FOUND', 'WEBHOOK_LIMIT_REACHED', 'WEBHOOK_URL_REJECTED')")
        return value
