...
class BulkBatteryUnitEventsResponse(BaseModel):
    """
    The partial-success report of a bulk telemetry ingest.
    """ # noqa: E501
    success: StrictBool
    message: StrictStr
    count: Annotated[int, Field(le=500, strict=True, ge=1)] = Field(description="How many records were accepted — equals `events.length`.")
    events: Annotated[List[BatteryUnitEventRow], Field(max_length=500)] = Field(description="The persisted rows, in the order the accepted records appeared in the request.")
    errors: List[StrictStr] = Field(description="Skipped records as `[index]`-prefixed reasons; empty when the whole batch was accepted.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "count", "events", "errors"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BulkBatteryUnitEventsResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BulkBatteryUnitEventsResponse from a dict"""
        if obj is None:
            return None
