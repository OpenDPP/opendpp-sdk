...
class PassportVcReadinessReport200Response(BaseModel):
    """
    ...
    page: StrictInt
    limit: StrictInt
    total: StrictInt = Field(description="Total non-archived passports in the catalog (the rollup base, not the page size).")
    total_pages: StrictInt = Field(alias="totalPages")
    ready: StrictInt = Field(description="Catalog-wide count of VC-ready SKUs (a facility with a country of production).")
    not_ready: StrictInt = Field(description="Catalog-wide count of not-yet-ready SKUs (= `total - ready`).", alias="notReady")
    results: List[PassportVcReadinessReport200ResponseResultsInner]
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "page", "limit", "total", "totalPages", "ready", "notReady", "results"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportVcReadinessReport200Response from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportVcReadinessReport200Response from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "results": [PassportVcReadinessReport200ResponseResultsInner.from_dict(_item) for _item in obj["results"]] if obj.get("results") is not None else None
        })
