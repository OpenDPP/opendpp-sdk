...
class WebhookSubscriptionWithSecret(BaseModel):
    """
    ...
    tenant_id: StrictStr = Field(description="Owning workspace (tenant) id.", alias="tenantId")
    url: StrictStr = Field(description="Receiver endpoint URL.")
    events: List[WebhookEventFilter] = Field(description="Event filters this subscription matches (validated at creation).")
    is_active: StrictBool = Field(description="`true` while the subscription receives deliveries; only active subscriptions are delivered to. Toggle with `PATCH /api/v1/webhooks/subscriptions/{id}` (`isActive`) to pause/resume without deleting.", alias="isActive")
    created_at: datetime = Field(alias="createdAt")
    updated_at: datetime = Field(alias="updatedAt")
    secret: Annotated[str, Field(strict=True)] = Field(description="Server-generated HMAC-SHA256 signing key (`whsec_` + 32 lowercase hex chars). Shown ONCE — here and in the `rotate-secret` response only; the list endpoint strips it. The FULL string, including the `whsec_` prefix, is the HMAC key for delivery signatures. Rotate it with `POST /api/v1/webhooks/subscriptions/{id}/rotate-secret`.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "tenantId", "url", "events", "isActive", "createdAt", "updatedAt", "secret"]

    @field_validator('secret')
    def secret_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^whsec_[0-9a-f]{32}$", value):
            raise ValueError(r"must validate the regular expression /^whsec_[0-9a-f]{32}$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebhookSubscriptionWithSecret from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebhookSubscriptionWithSecret from a dict"""
        if obj is None:
            return None
