...
class RegisterOperatorResponse(BaseModel):
    ...
    """ # noqa: E501
    success: StrictBool
    message: StrictStr = Field(description="Always `\"Economic Operator supplier registered successfully\"` (also when an existing operator was bound rather than created).")
    operator: OperatorRow
    warnings: List[AdvisoryItem] = Field(description="Non-blocking advisories. Carries a single EORI-not-found note when the OPT-IN EORI existence check is enabled and the supplied `eori` is not found in the EU EOS register. Empty `[]` otherwise. Never blocks registration.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "operator", "warnings"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RegisterOperatorResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RegisterOperatorResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "operator": OperatorRow.from_dict(obj["operator"]) if obj.get("operator") is not None else None,
