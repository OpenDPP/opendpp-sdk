...
class CreatePassport413Response(BaseModel):
    """
    ...
    def status_code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([413]):
            raise ValueError("must be one of enum values (413)")
        return value
    ...
    def code_validate_enum(cls, value):
        ...
        if value not in set(['FST_ERR_CTP_BODY_TOO_LARGE']):
            raise ValueError("must be one of enum values ('FST_ERR_CTP_BODY_TOO_LARGE')")
        return value

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Payload Too Large']):
            raise ValueError("must be one of enum values ('Payload Too Large')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreatePassport413Response from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreatePassport413Response from a dict"""
        if obj is None:
            return None
