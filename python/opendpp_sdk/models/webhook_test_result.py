...
class WebhookTestResult(BaseModel):
    ...
    """ # noqa: E501
    success: StrictBool = Field(description="The request was processed (NOT whether the receiver accepted it — see `delivered`).")
    event: StrictStr = Field(description="The event type sent.")
    url: StrictStr = Field(description="The receiver URL the test was sent to.")
    delivered: StrictBool = Field(description="`true` if the receiver returned a 2xx within the 5s timeout.")
    status_code: Optional[StrictInt] = Field(description="The receiver's HTTP status, or null on a transport/SSRF error.", alias="statusCode")
    error: Optional[StrictStr] = Field(description="Transport/SSRF error message when delivery failed, else null.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "event", "url", "delivered", "statusCode", "error"]
