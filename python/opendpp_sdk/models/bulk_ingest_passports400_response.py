...
BULKINGESTPASSPORTS400RESPONSE_ONE_OF_SCHEMAS = ["DefaultRequestRejectionErrorBody", "PassportBulkFailure"]

class BulkIngestPassports400Response(BaseModel):
    """
    BulkIngestPassports400Response
    """
    # data type: PassportBulkFailure
    oneof_schema_1_validator: Optional[PassportBulkFailure] = None
    # data type: DefaultRequestRejectionErrorBody
    oneof_schema_2_validator: Optional[DefaultRequestRejectionErrorBody] = None
    actual_instance: Optional[Union[DefaultRequestRejectionErrorBody, PassportBulkFailure]] = None
    one_of_schemas: Set[str] = { "DefaultRequestRejectionErrorBody", "PassportBulkFailure" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BulkIngestPassports400Response.model_construct()
        error_messages = []
        match = 0
        # validate data type: PassportBulkFailure
        if not isinstance(v, PassportBulkFailure):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PassportBulkFailure`")
        else:
            match += 1
        # validate data type: DefaultRequestRejectionErrorBody
        if not isinstance(v, DefaultRequestRejectionErrorBody):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DefaultRequestRejectionErrorBody`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BulkIngestPassports400Response with oneOf schemas: DefaultRequestRejectionErrorBody, PassportBulkFailure. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BulkIngestPassports400Response with oneOf schemas: DefaultRequestRejectionErrorBody, PassportBulkFailure. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PassportBulkFailure
        try:
            instance.actual_instance = PassportBulkFailure.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DefaultRequestRejectionErrorBody
        try:
            instance.actual_instance = DefaultRequestRejectionErrorBody.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BulkIngestPassports400Response with oneOf schemas: DefaultRequestRejectionErrorBody, PassportBulkFailure. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BulkIngestPassports400Response with oneOf schemas: DefaultRequestRejectionErrorBody, PassportBulkFailure. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DefaultRequestRejectionErrorBody, PassportBulkFailure]]:
