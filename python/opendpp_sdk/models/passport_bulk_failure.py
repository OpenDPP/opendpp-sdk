...
class PassportBulkFailure(BaseModel):
    """
    ...
    success: StrictBool
    error: StrictStr
    errors: List[StrictStr] = Field(description="One human-readable string per failed row, prefixed `[SKU: <productId>]` where the productId was readable.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "error", "errors"]

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Bulk Ingestion Failed']):
            raise ValueError("must be one of enum values ('Bulk Ingestion Failed')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportBulkFailure from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportBulkFailure from a dict"""
        if obj is None:
            return None
