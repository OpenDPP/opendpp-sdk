...
class PassportHistoryVersion(BaseModel):
    """
    PassportHistoryVersion
    """ # noqa: E501
    success: StrictBool
    passport_id: StrictStr = Field(alias="passportId")
    product_id: StrictStr = Field(alias="productId")
    var_date: Optional[datetime] = Field(default=None, description="Only on the by-date read: the instant that was asked for, normalised.", alias="date")
    version: Annotated[int, Field(strict=True, ge=1)] = Field(description="The version number; on the by-date read this is the live version number when `current` is true.")
    current: StrictBool = Field(description="`true` when the answer is the live passport (nothing changed since `date`), `false` for an archived version.")
    valid_from: Optional[datetime] = Field(description="The instant this version became current — the previous version's `validUntil`, or the passport's creation.", alias="validFrom")
    valid_until: Optional[datetime] = Field(description="The instant it stopped being current; `null` for the live passport.", alias="validUntil")
    recorded_at: Optional[datetime] = Field(description="When the snapshot was archived; `null` for the live passport.", alias="recordedAt")
    changed_by: Optional[StrictStr] = Field(description="Who recorded the change that replaced this version; `null` for the live passport.", alias="changedBy")
    change_reason: Optional[StrictStr] = Field(alias="changeReason")
    document_available: StrictBool = Field(description="`true` when this version is served as a document in `passport`. `false` only for a version archived before whole-row snapshots existed, where nothing can reconstruct the document — then `metadata` carries the regulated data instead and `passport` is `null`.", alias="documentAvailable")
    content_hash: Optional[StrictStr] = Field(description="SHA-256 over the RFC 8785 canonical form of this version's record, chained on the previous version's hash — the integrity evidence EN 18221:2026 §4.2 asks for, so a retrieved version can be VERIFIED and not merely read. `null` on a version archived before the column existed, and on the live version (which is the passport itself).", alias="contentHash")
    passport: Optional[Any] = None
    metadata: Optional[Any] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "passportId", "productId", "date", "version", "current", "validFrom", "validUntil", "recordedAt", "changedBy", "changeReason", "documentAvailable", "contentHash", "passport", "metadata"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportHistoryVersion from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportHistoryVersion from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "documentAvailable": obj.get("documentAvailable"),
