...
class GrantIssuedResponse(BaseModel):
    """
    ...
    success: StrictBool
    grant: GrantRow
    token: Annotated[str, Field(strict=True)] = Field(description="Legitimate-interest capability token (`dpp_li_` + 32 lowercase hex). Present it to public resolution endpoints as `Authorization: Bearer <token>` or `?grant=<token>`. Treat like a password — it cannot be retrieved again.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "grant", "token"]

    @field_validator('token')
    def token_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^dpp_li_[0-9a-f]{32}$", value):
            raise ValueError(r"must validate the regular expression /^dpp_li_[0-9a-f]{32}$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GrantIssuedResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GrantIssuedResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "grant": GrantRow.from_dict(obj["grant"]) if obj.get("grant") is not None else None,
            "token": obj.get("token")
        })
