...
class AasIngestCreated(BaseModel):
    """
    ...
    is_sealed: StrictBool = Field(description="True when the environment embedded an `eidasVerificationSeal` submodel (the seal is then stored on the passport).", alias="isSealed")
    signature_verified: StrictBool = Field(description="True when the embedded seal verified against the tenant's server-held signing public key. Always false for unsealed documents. (A sealed-but-unverified document never reaches 201 — it fails 400.)", alias="signatureVerified")
    vc_ready: StrictBool = Field(description="Whether the ingested passport can emit a UNTP Verifiable Credential — true only when a manufacturing facility with a country of production is linked. AAS ingestion does not set a facility, so a newly created passport is false; an in-place update preserves whatever facility the existing passport had.", alias="vcReady")
    ...
    warnings: List[ValidationErrorItem] = Field(description="Non-blocking advisories. Always present (empty array when none); carries the non-GS1 advisory when the resolved `productId` is not a GS1 GTIN-14/GRAI.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "passportId", "productId", "isSealed", "signatureVerified", "vcReady", "vcReadyReason", "warnings"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Digital Product Passport successfully ingested from AAS']):
            raise ValueError("must be one of enum values ('Digital Product Passport successfully ingested from AAS')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AasIngestCreated from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AasIngestCreated from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "signatureVerified": obj.get("signatureVerified"),
