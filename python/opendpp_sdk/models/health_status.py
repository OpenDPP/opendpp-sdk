...
class HealthStatus(BaseModel):
    ...
    status: StrictStr
    service: StrictStr
    timestamp: datetime = Field(description="Current server time, ISO 8601 UTC with milliseconds.")
    api_version: StrictStr = Field(description="SemVer of the public API contract currently served (equals the OpenAPI document's `info.version`; its MAJOR equals the `/api/v1` URL major).", alias="apiVersion")
    commit: StrictStr = Field(description="Short git commit SHA of the running build, or `\"unknown\"` when a build did not inject it.")
    built_at: StrictStr = Field(description="Build/deploy timestamp (ISO 8601 UTC), or `\"unknown\"` when a build did not inject it.", alias="builtAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["status", "service", "timestamp", "apiVersion", "commit", "builtAt"]
    ...
    def service_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OpenDPP B2B Enterprise Engine']):
            raise ValueError("must be one of enum values ('OpenDPP B2B Enterprise Engine')")
        return value
