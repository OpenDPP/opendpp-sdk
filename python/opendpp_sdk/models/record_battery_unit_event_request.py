...
class RecordBatteryUnitEventRequest(BaseModel):
    """
    ...
    """ # noqa: E501
    event_type: BatteryUnitEventType = Field(alias="eventType")
    state_of_health: Optional[Union[Annotated[float, Field(le=100, strict=True, ge=0)], Annotated[int, Field(le=100, strict=True, ge=0)]]] = Field(default=None, description="State of health, percent (0–100).", alias="stateOfHealth")
    cycle_count: Optional[Union[Annotated[float, Field(le=9007199254740991, strict=True, ge=0)], Annotated[int, Field(le=2147483647, strict=True, ge=0)]]] = Field(default=None, description="Cumulative full-equivalent cycles. Fractional values are accepted but truncated to an integer before persisting.", alias="cycleCount")
    remaining_capacity_ah: Optional[Union[Annotated[float, Field(le=9007199254740991, strict=True, ge=0)], Annotated[int, Field(le=2147483647, strict=True, ge=0)]]] = Field(default=None, description="Remaining capacity, ampere-hours.", alias="remainingCapacityAh")
    temperature_c: Optional[Union[Annotated[float, Field(le=10000, strict=True, ge=-273.15)], Annotated[int, Field(le=10000, strict=True, ge=-273)]]] = Field(default=None, description="Observed temperature, °C.", alias="temperatureC")
    payload: Optional[Any] = None
    recorded_at: Optional[RecordBatteryUnitEventRequestRecordedAt] = Field(default=None, alias="recordedAt")
    status: Optional[BatteryUnitStatus] = Field(default=None, description="Optional status transition, applied to the unit in the same transaction when it differs from the current status (works with any `eventType`; conventionally paired with `STATUS_CHANGE`). Transitioning to `RECYCLED` stamps `ceasedAt` (if not already set; never cleared) and turns the public unit view into a 410 tombstone; `RECYCLED` is terminal — once the unit's status is `RECYCLED` every further event is refused (400 `Terminal Unit Status`), so the status can never change again.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["eventType", "stateOfHealth", "cycleCount", "remainingCapacityAh", "temperatureC", "payload", "recordedAt", "status"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RecordBatteryUnitEventRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of recorded_at
        if self.recorded_at:
            _dict['recordedAt'] = self.recorded_at.to_dict()
        # puts key-value pairs in additional_properties in the top level
        if self.additional_properties is not None:
            for _key, _value in self.additional_properties.items():
                _dict[_key] = _value

        # set to None if state_of_health (nullable) is None
        # and model_fields_set contains the field
        if self.state_of_health is None and "state_of_health" in self.model_fields_set:
            _dict['stateOfHealth'] = None

        # set to None if cycle_count (nullable) is None
        # and model_fields_set contains the field
        if self.cycle_count is None and "cycle_count" in self.model_fields_set:
            _dict['cycleCount'] = None

        # set to None if remaining_capacity_ah (nullable) is None
        # and model_fields_set contains the field
        if self.remaining_capacity_ah is None and "remaining_capacity_ah" in self.model_fields_set:
            _dict['remainingCapacityAh'] = None

        # set to None if temperature_c (nullable) is None
        # and model_fields_set contains the field
        if self.temperature_c is None and "temperature_c" in self.model_fields_set:
            _dict['temperatureC'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RecordBatteryUnitEventRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "stateOfHealth": obj.get("stateOfHealth"),
            "cycleCount": obj.get("cycleCount"),
            "remainingCapacityAh": obj.get("remainingCapacityAh"),
            "temperatureC": obj.get("temperatureC"),
            "payload": obj.get("payload"),
            "recordedAt": RecordBatteryUnitEventRequestRecordedAt.from_dict(obj["recordedAt"]) if obj.get("recordedAt") is not None else None,
