...
class BatteryUnitRow(BaseModel):
    """
    ...
    """ # noqa: E501
    id: StrictStr
    serial_number: Annotated[str, Field(strict=True)] = Field(description="The battery's real physical serial number (GS1 AI-21 value). 1–20 URL-safe characters; GS1 recommends ≤ 20.", alias="serialNumber")
    digital_link_uri: StrictStr = Field(description="The unit's identifier URI. Under a GTIN-keyed passport, the GS1 Digital Link `{origin}/01/{gtin}/21/{serialNumber}` (AI 21 = the physical serial). Under any other passport — a GRAI, whose key admits no AI 21 qualifier, or a non-GS1 `productId` — an EN IEC 61406 Identification Link on the unit's own route, `{origin}/unit/{id}?.P={productId}&.S={serialNumber}` (EN 18219 Scheme 2, item level). Unique platform-wide.", alias="digitalLinkUri")
    passport_id: StrictStr = Field(description="The SKU/type-level passport this unit is an instance of.", alias="passportId")
    tenant_id: StrictStr = Field(description="Owning tenant id.", alias="tenantId")
    manufactured_at: Optional[datetime] = Field(alias="manufacturedAt")
    status: BatteryUnitStatus
    ceased_at: Optional[datetime] = Field(description="Stamped when the unit enters a terminal status — at creation for a unit created with initial status `RECYCLED`, or by the events endpoint on the `RECYCLED` transition; never cleared afterwards. Non-null means the public unit view is a 410 tombstone and the unit is refused as a `predecessorUnitId`.", alias="ceasedAt")
    predecessor_unit_id: Optional[StrictStr] = Field(description="Lineage: the original unit this battery was repurposed/remanufactured from (`null` for first-life units).", alias="predecessorUnitId")
    created_at: datetime = Field(alias="createdAt")
    updated_at: datetime = Field(alias="updatedAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "serialNumber", "digitalLinkUri", "passportId", "tenantId", "manufacturedAt", "status", "ceasedAt", "predecessorUnitId", "createdAt", "updatedAt"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitRow from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # set to None if predecessor_unit_id (nullable) is None
        # and model_fields_set contains the field
        if self.predecessor_unit_id is None and "predecessor_unit_id" in self.model_fields_set:
            _dict['predecessorUnitId'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitRow from a dict"""
        if obj is None:
            return None
