...
class PassportSealResponse(BaseModel):
    """
    ...
    """ # noqa: E501
    success: StrictBool = Field(description="Always true on 200.")
    message: StrictStr
    digital_seal: StrictStr = Field(description="Base64 ECDSA P-256 (prime256v1) SHA-256 signature over the metadata Merkle root (ADVANCED electronic seal — not a qualified seal, not a W3C DataIntegrityProof).", alias="digitalSeal")
    signing_public_key: StrictStr = Field(description="PEM-encoded public key of the tenant's signing key pair; the seal is offline-verifiable against `proof.merkleRoot` with standard ECDSA tooling.", alias="signingPublicKey")
    passport: PublicPassportJsonLd
    warnings: List[AdvisoryItem] = Field(description="Publish-time re-warning: a single non-GS1 advisory when the sealed passport's `productId` is not a GS1 GTIN/GRAI (its identifier is an EN IEC 61406 Identification Link, not a GS1 Digital Link — mint a GTIN you own via `POST /api/v1/gs1/gtin`). Empty `[]` when the productId is GS1-keyed. Non-blocking.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "digitalSeal", "signingPublicKey", "passport", "warnings"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Passport sealed with the tenant\'s eIDAS advanced electronic seal and published.']):
            raise ValueError("must be one of enum values ('Passport sealed with the tenant\'s eIDAS advanced electronic seal and published.')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportSealResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportSealResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "warnings": [AdvisoryItem.from_dict(_item) for _item in obj["warnings"]] if obj.get("warnings") is not None else None
        })
