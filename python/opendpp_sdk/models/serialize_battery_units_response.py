...
class SerializeBatteryUnitsResponse(BaseModel):
    """
    ...
    """ # noqa: E501
    success: StrictBool
    message: StrictStr = Field(description="E.g. `Serialised 2 individual unit(s)` or `Serialised 1 individual unit(s), skipped 1`.")
    count: Annotated[int, Field(strict=True, ge=1)] = Field(description="Number of units actually created (equals `units.length`).")
    units: List[BatteryUnitRow]
    errors: Optional[List[StrictStr]] = Field(default=None, description="Present only when some items were skipped — one plain-English string per skipped item, generally prefixed `[<serialNumber>]`.")
    warnings: List[AdvisoryItem] = Field(description="Non-blocking advisories. Carries a single note when the passport's `productId` is NOT a GS1 GTIN — the created units then have no scannable GS1 unit Digital Link (`/01/{gtin}/21/{serial}`) and resolve only via `/unit/{id}`. Empty `[]` for a GTIN-keyed passport.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "count", "units", "errors", "warnings"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SerializeBatteryUnitsResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SerializeBatteryUnitsResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "count": obj.get("count"),
            "units": [BatteryUnitRow.from_dict(_item) for _item in obj["units"]] if obj.get("units") is not None else None,
