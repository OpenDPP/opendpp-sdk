...
class PassportUpdateResponse(BaseModel):
    """
    ...
    message: StrictStr = Field(description="\"Draft published\" when a validated save promoted a DRAFT to ACTIVE; the longer message otherwise.")
    passport: PublicPassportJsonLd
    warnings: List[AdvisoryItem] = Field(description="Non-blocking advisories on a validated save: the data-carrier declaration checks (a symbology that does not fit the declared print attributes, a category whose carrier granularity does not match). Always `[]` when `draft: true`.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "passport", "warnings"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Draft published', 'Digital Product Passport successfully updated and history versioned']):
            raise ValueError("must be one of enum values ('Draft published', 'Digital Product Passport successfully updated and history versioned')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportUpdateResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportUpdateResponse from a dict"""
        if obj is None:
            return None
