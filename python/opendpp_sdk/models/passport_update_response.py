...
class PassportUpdateResponse(BaseModel):
    ...
    warnings: List[AdvisoryItem] = Field(description="Non-blocking advisories. Carries a single note when saving with `\"draft\": true` DEMOTED an already-published (ACTIVE/RECALLED/DECOMMISSIONED) passport to DRAFT — it is then no longer publicly resolvable. Empty `[]` otherwise.")
