...
GRANTREVOKEFORBIDDEN_ANY_OF_SCHEMAS = ["Error", "GrantRouteError"]

class GrantRevokeForbidden(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GrantRevokeForbidden.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GrantRevokeForbidden with anyOf schemas: Error, GrantRouteError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GrantRevokeForbidden with anyOf schemas: Error, GrantRouteError. Details: " + ", ".join(error_messages))
        else:
            return instance
