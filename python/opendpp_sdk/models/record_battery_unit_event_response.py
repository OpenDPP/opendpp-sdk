...
class RecordBatteryUnitEventResponse(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["success", "message", "event"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Dynamic data recorded']):
            raise ValueError("must be one of enum values ('Dynamic data recorded')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RecordBatteryUnitEventResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RecordBatteryUnitEventResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "event": BatteryUnitEventRow.from_dict(obj["event"]) if obj.get("event") is not None else None
        })
