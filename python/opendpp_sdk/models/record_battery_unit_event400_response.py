...
RECORDBATTERYUNITEVENT400RESPONSE_ONE_OF_SCHEMAS = ["Error", "FastifyDefaultBadRequest"]

class RecordBatteryUnitEvent400Response(BaseModel):
    """
    RecordBatteryUnitEvent400Response
    """
    # data type: Error
    oneof_schema_1_validator: Optional[Error] = None
    # data type: FastifyDefaultBadRequest
    oneof_schema_2_validator: Optional[FastifyDefaultBadRequest] = None
    actual_instance: Optional[Union[Error, FastifyDefaultBadRequest]] = None
    one_of_schemas: Set[str] = { "Error", "FastifyDefaultBadRequest" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = RecordBatteryUnitEvent400Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in RecordBatteryUnitEvent400Response with oneOf schemas: Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in RecordBatteryUnitEvent400Response with oneOf schemas: Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into RecordBatteryUnitEvent400Response with oneOf schemas: Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into RecordBatteryUnitEvent400Response with oneOf schemas: Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, FastifyDefaultBadRequest]]:
