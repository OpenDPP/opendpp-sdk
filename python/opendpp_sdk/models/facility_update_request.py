...
class FacilityUpdateRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="New name. Applied only when a non-empty string; empty/whitespace or non-string values are silently ignored (the name can never be cleared).")
    activity: Optional[StrictStr] = Field(default=None, description="New activity, or null/\"\" to clear. A whitespace-only string is stored as \"\" (see schema description).")
    street_address: Optional[StrictStr] = Field(default=None, description="New street address, or null/\"\" to clear. A whitespace-only string is stored as \"\" (see schema description).", alias="streetAddress")
    city: Optional[StrictStr] = Field(default=None, description="New city, or null/\"\" to clear. A whitespace-only string is stored as \"\" (see schema description).")
    postal_code: Optional[StrictStr] = Field(default=None, description="New postal code, or null/\"\" to clear. A whitespace-only string is stored as \"\" (see schema description).", alias="postalCode")
    country: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="New 2-letter ISO country code (400 if a string that does not match; stored uppercase; non-string values are silently ignored).")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["name", "activity", "streetAddress", "city", "postalCode", "country"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FacilityUpdateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FacilityUpdateRequest from a dict"""
        if obj is None:
            return None
