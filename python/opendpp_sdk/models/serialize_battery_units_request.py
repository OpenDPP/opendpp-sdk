...
SERIALIZEBATTERYUNITSREQUEST_ANY_OF_SCHEMAS = ["BatteryUnitCreateItem", "SerializeBatteryUnitsRequestAnyOf"]

class SerializeBatteryUnitsRequest(BaseModel):
    """
    ...
    # data type: BatteryUnitCreateItem
    anyof_schema_1_validator: Optional[BatteryUnitCreateItem] = None
    # data type: SerializeBatteryUnitsRequestAnyOf
    anyof_schema_2_validator: Optional[SerializeBatteryUnitsRequestAnyOf] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[BatteryUnitCreateItem, SerializeBatteryUnitsRequestAnyOf]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "BatteryUnitCreateItem", "SerializeBatteryUnitsRequestAnyOf" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = SerializeBatteryUnitsRequest.model_construct()
        error_messages = []
        # validate data type: BatteryUnitCreateItem
        if not isinstance(v, BatteryUnitCreateItem):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BatteryUnitCreateItem`")
        else:
            return v

        # validate data type: SerializeBatteryUnitsRequestAnyOf
        if not isinstance(v, SerializeBatteryUnitsRequestAnyOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SerializeBatteryUnitsRequestAnyOf`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in SerializeBatteryUnitsRequest with anyOf schemas: BatteryUnitCreateItem, SerializeBatteryUnitsRequestAnyOf. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[BatteryUnitCreateItem] = None
        try:
            instance.actual_instance = BatteryUnitCreateItem.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[SerializeBatteryUnitsRequestAnyOf] = None
        try:
            instance.actual_instance = SerializeBatteryUnitsRequestAnyOf.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SerializeBatteryUnitsRequest with anyOf schemas: BatteryUnitCreateItem, SerializeBatteryUnitsRequestAnyOf. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BatteryUnitCreateItem, SerializeBatteryUnitsRequestAnyOf]]:
