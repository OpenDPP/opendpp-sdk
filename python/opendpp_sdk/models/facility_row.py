...
class FacilityRow(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Facility id (UUID).")
    gln: Annotated[str, Field(strict=True)] = Field(description="GS1 Global Location Number — 13 digits with a valid GS1 modulo-10 check digit. Unique platform-wide. Immutable after registration (it is the resolvable UFI).")
    name: StrictStr = Field(description="Facility display name (trimmed, non-empty).")
    activity: Optional[StrictStr] = Field(description="Free-text activity, e.g. \"Cell assembly\", \"Final manufacturing\", \"Recycling\". Public in JSON-LD; not emitted in the AAS export.")
    street_address: Optional[StrictStr] = Field(description="Street address. Owner-only: redacted from public JSON-LD and never emitted in AAS.", alias="streetAddress")
    city: Optional[StrictStr] = Field(description="City. Owner-only: redacted from public JSON-LD and never emitted in AAS.")
    postal_code: Optional[StrictStr] = Field(description="Postal code. Owner-only: redacted from public JSON-LD and never emitted in AAS.", alias="postalCode")
    country: Annotated[str, Field(strict=True)] = Field(description="2-letter ISO 3166-1 alpha-2 country code, stored uppercase. Public in both JSON-LD and AAS.")
    operator_id: Optional[StrictStr] = Field(description="Id of the owning Economic Operator, or null for a tenant-level facility. Set at creation; not updatable via PUT.", alias="operatorId")
    tenant_id: StrictStr = Field(description="Owning tenant workspace id.", alias="tenantId")
    created_at: datetime = Field(alias="createdAt")
    updated_at: datetime = Field(alias="updatedAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "gln", "name", "activity", "streetAddress", "city", "postalCode", "country", "operatorId", "tenantId", "createdAt", "updatedAt"]

    @field_validator('gln')
    def gln_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[0-9]{13}$", value):
            raise ValueError(r"must validate the regular expression /^[0-9]{13}$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FacilityRow from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FacilityRow from a dict"""
        if obj is None:
            return None
