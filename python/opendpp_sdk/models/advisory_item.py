...
class AdvisoryItem(BaseModel):
    ...
    """ # noqa: E501
    code: StrictStr = Field(description="Stable advisory code. WARNINGS: `NON_GS1_PRODUCT_ID` (the productId is not a GS1 GTIN/GRAI → issued as an Identification Link, not a GS1 Digital Link), `PII_SHAPE_DETECTED` (metadata looks like personal data), `UNIT_NO_SCANNABLE_LINK` (units under a non-GTIN passport carry an Identification Link, not a GS1 unit Digital Link), `EORI_NOT_FOUND` (a declared EORI was not in the EU EOS register), `CARRIER_SYMBOLOGY_NOT_RENDERED` (the declared EN 18220 Clause 6 carrier is a symbology this node does not encode, so the symbol served is a QR Code), `CATEGORY_GRANULARITY_UNEXPECTED` (the passport's identification granularity differs from the level the product group's own instrument requires; EN 18220 §4.2 leaves that level to sector legislation). NOTICES: `OPERATOR_AUTO_ATTRIBUTED` (operatorId omitted → the workspace's first bound operator was used), `GTIN_AUTO_COPIED` (a valid GTIN-14/GRAI productId was copied into metadata.gtin/metadata.grai).")
    path: Optional[StrictStr] = Field(default=None, description="The field the advisory is about (e.g. `productId`, `draft`, `regId`), when applicable.")
    message: StrictStr = Field(description="Developer-facing detail (English).")
    friendly_message: StrictStr = Field(description="End-user-facing, localizable summary.", alias="friendlyMessage")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["code", "path", "message", "friendlyMessage"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NON_GS1_PRODUCT_ID', 'PII_SHAPE_DETECTED', 'UNIT_NO_SCANNABLE_LINK', 'EORI_NOT_FOUND', 'CARRIER_SYMBOLOGY_NOT_RENDERED', 'CATEGORY_GRANULARITY_UNEXPECTED', 'OPERATOR_AUTO_ATTRIBUTED', 'GTIN_AUTO_COPIED']):
            raise ValueError("must be one of enum values ('NON_GS1_PRODUCT_ID', 'PII_SHAPE_DETECTED', 'UNIT_NO_SCANNABLE_LINK', 'EORI_NOT_FOUND', 'CARRIER_SYMBOLOGY_NOT_RENDERED', 'CATEGORY_GRANULARITY_UNEXPECTED', 'OPERATOR_AUTO_ATTRIBUTED', 'GTIN_AUTO_COPIED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AdvisoryItem from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AdvisoryItem from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "friendlyMessage": obj.get("friendlyMessage")
        })
