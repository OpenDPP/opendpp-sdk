...
class WebhookDeliveryRow(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Outbox record id.")
    event: StrictStr = Field(description="Event type, e.g. `passport.sealed`.")
    status: StrictStr = Field(description="Overall delivery state. DELIVERED means the event reached every matching endpoint. NO_SUBSCRIBERS means no active subscription filtered for this event, so nothing was sent. FAILED after six exhausted attempts (dead-lettered), or when the last matching subscription was removed while the event was still failing.")
    retry_count: StrictInt = Field(description="Failed attempts so far (0–5).", alias="retryCount")
    last_attempt: Optional[datetime] = Field(description="Timestamp of the most recent attempt, or null if never attempted.", alias="lastAttempt")
    next_retry_at: Optional[datetime] = Field(description="When the next retry is eligible (null if delivered or dead-lettered).", alias="nextRetryAt")
    error_message: Optional[StrictStr] = Field(description="Joined per-endpoint error text from the last failed attempt, or null.", alias="errorMessage")
    created_at: datetime = Field(alias="createdAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "event", "status", "retryCount", "lastAttempt", "nextRetryAt", "errorMessage", "createdAt"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PENDING', 'DELIVERED', 'FAILED', 'NO_SUBSCRIBERS']):
            raise ValueError("must be one of enum values ('PENDING', 'DELIVERED', 'FAILED', 'NO_SUBSCRIBERS')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebhookDeliveryRow from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # set to None if last_attempt (nullable) is None
        # and model_fields_set contains the field
        if self.last_attempt is None and "last_attempt" in self.model_fields_set:
            _dict['lastAttempt'] = None

        # set to None if next_retry_at (nullable) is None
        # and model_fields_set contains the field
        if self.next_retry_at is None and "next_retry_at" in self.model_fields_set:
            _dict['nextRetryAt'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebhookDeliveryRow from a dict"""
        if obj is None:
            return None
