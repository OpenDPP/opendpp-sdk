...
BATTERYUNITEVENTBADREQUEST_ONE_OF_SCHEMAS = ["DefaultRequestRejectionError", "Error"]

class BatteryUnitEventBadRequest(BaseModel):
    """
    ...
    actual_instance: Optional[Union[DefaultRequestRejectionError, Error]] = None
    one_of_schemas: Set[str] = { "DefaultRequestRejectionError", "Error" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BatteryUnitEventBadRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BatteryUnitEventBadRequest with oneOf schemas: DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BatteryUnitEventBadRequest with oneOf schemas: DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BatteryUnitEventBadRequest with oneOf schemas: DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BatteryUnitEventBadRequest with oneOf schemas: DefaultRequestRejectionError, Error. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DefaultRequestRejectionError, Error]]:
