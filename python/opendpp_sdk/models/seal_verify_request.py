...
class SealVerifyRequest(BaseModel):
    ...
    """ # noqa: E501
    payload: SealVerifyRequestPayload
    signature: Optional[StrictStr] = Field(default=None, description="Base64 ECDSA (P-256 / SHA-256) seal signature. Optional when `payload.proof.proofValue` (or `signatureValue`) is present.")
    public_key: Optional[StrictStr] = Field(default=None, description="PEM (SPKI) public key of the sealing tenant. Optional when `payload.proof.publicKeyPem` or an `x5c` chain is present. CRLF line endings are normalized before matching.", alias="publicKey")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["payload", "signature", "publicKey"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SealVerifyRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SealVerifyRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "payload": SealVerifyRequestPayload.from_dict(obj["payload"]) if obj.get("payload") is not None else None,
