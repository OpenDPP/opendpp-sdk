...
class RotateTenantKeysResponse(BaseModel):
    """
    Confirmation that the workspace's signing key was rotated, returning the new public key.
    """ # noqa: E501
    success: StrictBool
    message: StrictStr = Field(description="Outcome message. On a first provisioning: `\"eIDAS Asymmetric Key Pair generated in secure DB custody successfully.\"` On a rotation, a message noting that the previous key is retired but retained in your DID document so existing credentials keep verifying.")
    public_key: StrictStr = Field(description="The new ECDSA prime256v1 (P-256) public key, PEM-encoded (SPKI, `-----BEGIN PUBLIC KEY-----` block, trailing newline). The matching private key is held only AES-256-GCM-encrypted in the platform vault and is never returned.", alias="publicKey")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "publicKey"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RotateTenantKeysResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RotateTenantKeysResponse from a dict"""
        if obj is None:
            return None
