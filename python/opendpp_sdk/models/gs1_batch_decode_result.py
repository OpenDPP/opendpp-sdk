...
GS1BATCHDECODERESULT_ONE_OF_SCHEMAS = ["Gs1BatchDecodeError", "Gs1BatchDecodeOk"]

class Gs1BatchDecodeResult(BaseModel):
    """
    ...
    """
    # data type: Gs1BatchDecodeOk
    oneof_schema_1_validator: Optional[Gs1BatchDecodeOk] = None
    # data type: Gs1BatchDecodeError
    oneof_schema_2_validator: Optional[Gs1BatchDecodeError] = None
    actual_instance: Optional[Union[Gs1BatchDecodeError, Gs1BatchDecodeOk]] = None
    one_of_schemas: Set[str] = { "Gs1BatchDecodeError", "Gs1BatchDecodeOk" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = Gs1BatchDecodeResult.model_construct()
        error_messages = []
        match = 0
        # validate data type: Gs1BatchDecodeOk
        if not isinstance(v, Gs1BatchDecodeOk):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Gs1BatchDecodeOk`")
        else:
            match += 1
        # validate data type: Gs1BatchDecodeError
        if not isinstance(v, Gs1BatchDecodeError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Gs1BatchDecodeError`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in Gs1BatchDecodeResult with oneOf schemas: Gs1BatchDecodeError, Gs1BatchDecodeOk. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in Gs1BatchDecodeResult with oneOf schemas: Gs1BatchDecodeError, Gs1BatchDecodeOk. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Gs1BatchDecodeOk
        try:
            instance.actual_instance = Gs1BatchDecodeOk.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into Gs1BatchDecodeError
        try:
            instance.actual_instance = Gs1BatchDecodeError.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into Gs1BatchDecodeResult with oneOf schemas: Gs1BatchDecodeError, Gs1BatchDecodeOk. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Gs1BatchDecodeResult with oneOf schemas: Gs1BatchDecodeError, Gs1BatchDecodeOk. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Gs1BatchDecodeError, Gs1BatchDecodeOk]]:
