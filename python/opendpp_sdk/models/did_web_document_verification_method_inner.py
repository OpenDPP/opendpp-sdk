...
class DidWebDocumentVerificationMethodInner(BaseModel):
    """
    DidWebDocumentVerificationMethodInner
    """ # noqa: E501
    id: StrictStr = Field(description="`<did>#key-<index>` — matches the credential `kid`.")
    type: StrictStr
    controller: StrictStr
    public_key_jwk: Dict[str, Any] = Field(description="The public key as a JWK (EC P-256, `alg: ES256`, `use: sig`). Public material only.", alias="publicKeyJwk")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "controller", "publicKeyJwk"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['JsonWebKey2020']):
            raise ValueError("must be one of enum values ('JsonWebKey2020')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DidWebDocumentVerificationMethodInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DidWebDocumentVerificationMethodInner from a dict"""
        if obj is None:
            return None
