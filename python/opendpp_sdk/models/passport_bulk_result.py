...
class PassportBulkResult(BaseModel):
    """
    ...
    """ # noqa: E501
    success: StrictBool
    message: StrictStr = Field(description="Template: \"Bulk CSV ingestion finished. Registered <n> passports, skipped <m> rows with errors.\"")
    inserted_count: Annotated[int, Field(strict=True, ge=0)] = Field(description="Number of rows actually inserted (= `results.length`).", alias="insertedCount")
    results: List[PassportBulkResultResultsInner]
    errors: Optional[List[StrictStr]] = Field(default=None, description="Human-readable per-row failure strings, prefixed `[SKU: <productId>]` (or \"Missing or invalid productId in spreadsheet row\"). Present ONLY when at least one row failed — omitted otherwise.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "message", "insertedCount", "results", "errors"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportBulkResult from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportBulkResult from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "insertedCount": obj.get("insertedCount"),
            "results": [PassportBulkResultResultsInner.from_dict(_item) for _item in obj["results"]] if obj.get("results") is not None else None,
