...
class UntpEventProof(BaseModel):
    """
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="MUST be `DataIntegrityProof`.")
    cryptosuite: StrictStr = Field(description="MUST be `ecdsa-jcs-2019` (RFC 8785 JCS).")
    created: Optional[datetime] = None
    proof_purpose: Optional[StrictStr] = Field(default=None, description="e.g. `assertionMethod`.", alias="proofPurpose")
    verification_method: Optional[UntpEventProofVerificationMethod] = Field(default=None, alias="verificationMethod")
    proof_value: StrictStr = Field(description="Multibase base58btc (`z…`) ecdsa-jcs-2019 signature. Stored verbatim with the event.", alias="proofValue")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "cryptosuite", "created", "proofPurpose", "verificationMethod", "proofValue"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DataIntegrityProof']):
            raise ValueError("must be one of enum values ('DataIntegrityProof')")
        return value

    @field_validator('cryptosuite')
    def cryptosuite_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ecdsa-jcs-2019']):
            raise ValueError("must be one of enum values ('ecdsa-jcs-2019')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UntpEventProof from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.verification_method:
            _dict['verificationMethod'] = self.verification_method.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UntpEventProof from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "verificationMethod": UntpEventProofVerificationMethod.from_dict(obj["verificationMethod"]) if obj.get("verificationMethod") is not None else None,
