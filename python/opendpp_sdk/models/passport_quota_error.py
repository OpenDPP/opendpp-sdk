...
class PassportQuotaError(BaseModel):
    """
    ...
    """ # noqa: E501
    success: Optional[StrictBool] = Field(default=None, description="Always `false` when present.")
    error: StrictStr = Field(description="Short error title.")
    message: StrictStr = Field(description="Human-readable explanation.")
    code: Optional[StrictStr] = Field(default=None, description="Machine-readable discriminator. `passport_quota_exceeded` = the tier's passport cap is reached; `api_access_required` = a programmatic (API-key) write was attempted on a tier without the API-access entitlement (use the dashboard session, or upgrade to a Growth+ plan); omitted for a lapsed-subscription 402.")
    quota: Optional[PassportQuotaErrorQuota] = None
    upgrade_url: Optional[StrictStr] = Field(default=None, description="Where the workspace owner can upgrade the plan.", alias="upgradeUrl")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "error", "message", "code", "quota", "upgradeUrl"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportQuotaError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportQuotaError from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "code": obj.get("code"),
            "quota": PassportQuotaErrorQuota.from_dict(obj["quota"]) if obj.get("quota") is not None else None,
