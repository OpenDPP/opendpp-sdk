...
class BatteryUnitCreateItem(BaseModel):
    """
    ...
    """ # noqa: E501
    serial_number: Annotated[str, Field(strict=True)] = Field(description="Required. The battery's real physical serial (GS1 AI-21). Trimmed, then must match `^[A-Za-z0-9._-]{1,20}$` (GS1 AI-21's 20-character maximum) — a longer serial is rejected at ingest with a per-item error. Must be unique within the passport (duplicates are skipped with an item error).", alias="serialNumber")
    manufactured_at: Optional[BatteryUnitCreateItemManufacturedAt] = Field(default=None, alias="manufacturedAt")
    status: Optional[BatteryUnitStatus] = Field(default=None, description="Optional initial status. Defaults to `IN_SERVICE`. Creating a unit directly with `RECYCLED` records an already-ceased battery: `ceasedAt` is stamped at creation, the public view is a 410 tombstone, and the unit is refused as a `predecessorUnitId`.")
    predecessor_unit_id: Optional[StrictStr] = Field(default=None, description="Optional lineage linkage: id of an existing unit **in your tenant** (any passport) that this battery was repurposed/remanufactured from. A recycled predecessor is refused — the check keys on terminality (`ceasedAt` set or a terminal `status`), however the unit reached it. Atomically with creation, a `STATUS_CHANGE` event (`{status, successorUnitId, successorSerial}`) is appended to the predecessor and its status set to `predecessorStatus`.", alias="predecessorUnitId")
    predecessor_status: Optional[StrictStr] = Field(default='REPURPOSED', description="Optional; only meaningful with `predecessorUnitId`. The status the predecessor transitions to. Defaults to `REPURPOSED`.", alias="predecessorStatus")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["serialNumber", "manufacturedAt", "status", "predecessorUnitId", "predecessorStatus"]
    ...
    @field_validator('predecessor_status')
    def predecessor_status_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['REPURPOSED', 'REMANUFACTURED', 'REUSED']):
            raise ValueError("must be one of enum values ('REPURPOSED', 'REMANUFACTURED', 'REUSED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitCreateItem from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of manufactured_at
        if self.manufactured_at:
            _dict['manufacturedAt'] = self.manufactured_at.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitCreateItem from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "manufacturedAt": BatteryUnitCreateItemManufacturedAt.from_dict(obj["manufacturedAt"]) if obj.get("manufacturedAt") is not None else None,
            "status": obj.get("status"),
            "predecessorUnitId": obj.get("predecessorUnitId"),
            "predecessorStatus": obj.get("predecessorStatus") if obj.get("predecessorStatus") is not None else 'REPURPOSED'
        })
