...
class WebhookSubscriptionCreateRequest(BaseModel):
    ...
    """ # noqa: E501
    url: StrictStr = Field(description="Absolute **https** endpoint URL of your receiver (e.g. a PLM/ERP integration endpoint). Cleartext `http://` targets are rejected with 400: the delivered payload is the full passport document, so the transport is encrypted or the subscription is refused. DNS-resolved and SSRF-guarded at registration too: malformed URLs, loopback, private (RFC 1918/CGNAT), link-local/cloud-metadata, multicast, and equivalent IPv6 ranges are rejected with 400. Redirects are never followed at delivery time.")
    events: Annotated[List[WebhookEventFilter], Field(min_length=1)] = Field(description="Non-empty list of event filters. Any value outside the allowed set is rejected with 400.")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WebhookSubscriptionCreateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WebhookSubscriptionCreateRequest from a dict"""
        if obj is None:
            return None
