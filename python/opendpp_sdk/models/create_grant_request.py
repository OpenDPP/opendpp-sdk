...
class CreateGrantRequest(BaseModel):
    ...
    """ # noqa: E501
    grantee_name: Annotated[str, Field(min_length=1, strict=True, max_length=160)] = Field(description="Required (whitespace-only is rejected as missing). Truncated to 160 characters.", alias="granteeName")
    grantee_email: Optional[Annotated[str, Field(strict=True, max_length=254)]] = Field(default=None, description="Optional. Truncated to 254 characters, then must match a basic e-mail pattern (`x@y.z`) or the request fails with 400 `granteeEmail is invalid`. Stored as given (not lowercased).", alias="granteeEmail")
    organization: Optional[Annotated[str, Field(strict=True, max_length=200)]] = Field(default=None, description="Optional. Truncated to 200 characters.")
    purpose: Optional[Annotated[str, Field(strict=True, max_length=2000)]] = Field(default=None, description="Optional stated legitimate interest. Truncated to 2000 characters.")
    scope_type: StrictStr = Field(description="Required. `UNIT` needs `batteryUnitId`; `PASSPORT` needs `passportId`; `TENANT` is workspace-wide. Any other value ⇒ 400.", alias="scopeType")
    passport_id: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(default=None, description="Required when `scopeType` is `PASSPORT`; ignored otherwise. Must be a non-DRAFT passport in this workspace — a missing, cross-tenant, or `DRAFT` id (or omitting the field entirely) returns 404.", alias="passportId")
    battery_unit_id: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(default=None, description="Required when `scopeType` is `UNIT`; ignored otherwise. Must be a battery unit in this workspace — a missing or cross-tenant id (or omitting the field entirely) returns 404.", alias="batteryUnitId")
    expires_at: datetime = Field(description="Required. Date parsing is lenient (some non-ISO values may be accepted) — ISO 8601 is strongly recommended. Must be in the future and at most 366 days from now.", alias="expiresAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["granteeName", "granteeEmail", "organization", "purpose", "scopeType", "passportId", "batteryUnitId", "expiresAt"]
