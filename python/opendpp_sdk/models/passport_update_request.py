...
class PassportUpdateRequest(BaseModel):
    ...
    draft: Optional[StrictBool] = Field(default=False, description="true = save as draft: skips ESPR validation and forces status DRAFT (this also demotes an already-published passport back to DRAFT). false/absent = validated save; a DRAFT passport is published (status ACTIVE, emits the passport.ingested webhook).")
    ...
    __properties: ClassVar[List[str]] = ["metadata", "draft", "changeReason", "facilityId", "enrichment"]
