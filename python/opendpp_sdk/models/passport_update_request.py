...
class PassportUpdateRequest(BaseModel):
    ...
    """ # noqa: E501
    metadata: Dict[str, Any] = Field(description="Full replacement ESPR metadata object. `metadata.category` must be one of: textiles, batteries, electronics, cosmetics, toys, iron-steel, aluminium, chemicals, construction. Validated against the category's compliance rules unless `draft: true` (400 with `errors[]` on failure — no `warnings` on this route). A machine-readable JSON Schema is published at GET /api/v1/schemas/{category} for **textiles, batteries, electronics, chemicals and construction only** — cosmetics, toys, iron-steel and aluminium are validated by built-in rules and return 404 from that endpoint.")
    draft: Optional[StrictBool] = Field(default=False, description="true = save as draft: skips ESPR validation and keeps status DRAFT; refused with 409 `DRAFT_DEMOTION_REFUSED` on an already-published passport (publishing is one-way). false/absent = validated save; a DRAFT passport is published (status ACTIVE, emits the passport.ingested webhook).")
    change_reason: Optional[StrictStr] = Field(default=None, description="Free-text reason recorded on the version-history snapshot. Defaults to \"API Update\".", alias="changeReason")
    facility_id: Optional[StrictStr] = Field(default=None, description="GLN-backed facility assignment. Omit to leave unchanged; null or \"\" detaches; a UUID attaches a facility that must be owned by your tenant (otherwise 400).", alias="facilityId")
    enrichment: Optional[PassportEnrichmentInput] = None
    carrier: Optional[CarrierDeclaration] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["metadata", "draft", "changeReason", "facilityId", "enrichment", "carrier"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportUpdateRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.facility_id is None and "facility_id" in self.model_fields_set:
            _dict['facilityId'] = None

        # set to None if enrichment (nullable) is None
        # and model_fields_set contains the field
        if self.enrichment is None and "enrichment" in self.model_fields_set:
            _dict['enrichment'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportUpdateRequest from a dict"""
        if obj is None:
            return None
