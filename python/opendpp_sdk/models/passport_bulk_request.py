...
class PassportBulkRequest(BaseModel):
    """
    A batch of passports to ingest, with optional dry-run preview and upsert-on-conflict behaviour.
    """ # noqa: E501
    passports: Annotated[List[PassportBulkRow], Field(min_length=1, max_length=200)] = Field(description="1–200 rows. The bounds are enforced before any row is processed; violations return the default `{statusCode, code, error, message}` error body.")
    dry_run: Optional[StrictBool] = Field(default=None, description="When `true`, every row is validated and the duplicate/operator/facility checks run, but **nothing is written** — the response is **200** and reports which rows are OK (`results`) vs failed (`errors[]`). Powers a pre-import preview.", alias="dryRun")
    upsert: Optional[StrictBool] = Field(default=None, description="When `true`, a row whose `(productId, operator)` already exists **updates** the existing passport instead of being reported as a duplicate (a sealed passport is never overwritten). Enables idempotent re-import of a corrected catalog.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["passports", "dryRun", "upsert"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportBulkRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in passports (list)
        _items = []
        if self.passports:
            for _item_passports in self.passports:
                if _item_passports:
                    _items.append(_item_passports.to_dict())
            _dict['passports'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportBulkRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "passports": [PassportBulkRow.from_dict(_item) for _item in obj["passports"]] if obj.get("passports") is not None else None,
