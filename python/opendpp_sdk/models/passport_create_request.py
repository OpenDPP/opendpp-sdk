...
class PassportCreateRequest(BaseModel):
    ...
    """ # noqa: E501
    product_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Product identifier: a GTIN-14 (exactly 14 digits with a valid GS1 mod-10 check digit — auto-copied to `metadata.gtin`), a GRAI (14-digit numeric asset id with valid check digit + optional up to 16 alphanumeric serial chars, total 14–30 — auto-copied to `metadata.grai`), or a free-form SKU. Determines the identifier URI: the GS1 Application Identifier (`01` vs `8003`) of the Digital Link, or an Identification Link for a non-GS1 SKU. Characters must be ISO/IEC 646 (7-bit ASCII; EN 18219 §4.3.2) — anything else, and whitespace-only values, are rejected 400. Unique per economic operator (409 on duplicate).", alias="productId")
    operator_id: Optional[StrictStr] = Field(default=None, description="UUID of an EconomicOperator bound to your tenant workspace (403 if not bound). Defaults to your workspace's first bound operator. Operator-scoped API keys force their own operator (403 on mismatch).", alias="operatorId")
    facility_id: Optional[StrictStr] = Field(default=None, description="Optional UUID of a Facility (GLN-backed Unique Facility Identifier) in your workspace; 400 if not found.", alias="facilityId")
    metadata: PassportMetadataInput
    draft: Optional[StrictBool] = Field(default=False, description="When true: skips ALL ESPR validation, stores the passport with `status: \"DRAFT\"` (not publicly resolvable), and emits no webhook. Publish later via a validated edit.")
    enrichment: Optional[PassportEnrichmentInput] = None
    carrier: Optional[CarrierDeclaration] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["productId", "operatorId", "facilityId", "metadata", "draft", "enrichment", "carrier"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportCreateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportCreateRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "metadata": PassportMetadataInput.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None,
            "draft": obj.get("draft") if obj.get("draft") is not None else False,
            "enrichment": PassportEnrichmentInput.from_dict(obj["enrichment"]) if obj.get("enrichment") is not None else None,
            "carrier": CarrierDeclaration.from_dict(obj["carrier"]) if obj.get("carrier") is not None else None
        })
