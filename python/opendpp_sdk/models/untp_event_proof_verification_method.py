...
UNTPEVENTPROOFVERIFICATIONMETHOD_ONE_OF_SCHEMAS = ["UntpVerificationMethod", "str"]

class UntpEventProofVerificationMethod(BaseModel):
    """
    Either a key-identifier string or an embedded object carrying an `x5c` certificate chain.
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="Public-key identifier / DID URL.")
    # data type: UntpVerificationMethod
    oneof_schema_2_validator: Optional[UntpVerificationMethod] = None
    actual_instance: Optional[Union[UntpVerificationMethod, str]] = None
    one_of_schemas: Set[str] = { "UntpVerificationMethod", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = UntpEventProofVerificationMethod.model_construct()
        error_messages = []
        match = 0
        ...
        # validate data type: UntpVerificationMethod
        if not isinstance(v, UntpVerificationMethod):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UntpVerificationMethod`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in UntpEventProofVerificationMethod with oneOf schemas: UntpVerificationMethod, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in UntpEventProofVerificationMethod with oneOf schemas: UntpVerificationMethod, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into UntpVerificationMethod
        try:
            instance.actual_instance = UntpVerificationMethod.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into UntpEventProofVerificationMethod with oneOf schemas: UntpVerificationMethod, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UntpEventProofVerificationMethod with oneOf schemas: UntpVerificationMethod, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], UntpVerificationMethod, str]]:
