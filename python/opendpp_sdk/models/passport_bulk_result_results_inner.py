...
class PassportBulkResultResultsInner(BaseModel):
    ...
    warnings: Optional[List[AdvisoryItem]] = Field(default=None, description="Per-row non-blocking advisories — the non-GS1 \"no scannable QR\" note and the PII-shape privacy advisory. Empty `[]` when the row is clean; never blocks the row.")
