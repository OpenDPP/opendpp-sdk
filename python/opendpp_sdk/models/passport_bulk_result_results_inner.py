...
class PassportBulkResultResultsInner(BaseModel):
    """
    PassportBulkResultResultsInner
    """ # noqa: E501
    product_id: StrictStr = Field(alias="productId")
    digital_link_uri: StrictStr = Field(description="Generated GS1 Digital Link URI `https://opendpp-node.eu/{01|8003}/{productId}`.", alias="digitalLinkUri")
    vc_ready: StrictBool = Field(description="Whether this row's passport can emit a UNTP Verifiable Credential — true only when a manufacturing facility with a country of production is linked. On a `dryRun` preview this reflects the EFFECTIVE facility after import (the row's facility, else the existing passport's preserved one).", alias="vcReady")
    ...
    warnings: Optional[List[AdvisoryItem]] = Field(default=None, description="Per-row non-blocking advisories — the non-GS1 \"Identification Link, not a GS1 Digital Link\" note and the PII-shape privacy advisory. Empty `[]` when the row is clean; never blocks the row.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["productId", "digitalLinkUri", "vcReady", "vcReadyReason", "warnings"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PassportBulkResultResultsInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PassportBulkResultResultsInner from a dict"""
        if obj is None:
            return None
