...
class FacilityCreateRequest(BaseModel):
    ...
    """ # noqa: E501
    gln: Annotated[str, Field(strict=True)] = Field(description="GS1 GLN-13. Trimmed, then validated: exactly 13 digits with a valid GS1 modulo-10 check digit. Unique platform-wide (409 on duplicate). Immutable after registration.")
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Facility name. Must be a non-empty string; stored trimmed.")
    country: Annotated[str, Field(strict=True)] = Field(description="2-letter ISO country code (case-insensitive on input; stored uppercase).")
    activity: Optional[StrictStr] = Field(default=None, description="Optional activity, e.g. \"Cell assembly\". Trimmed; empty/whitespace is stored as null.")
    street_address: Optional[StrictStr] = Field(default=None, description="Optional street address (owner-only in public views). Trimmed; empty is stored as null.", alias="streetAddress")
    city: Optional[StrictStr] = Field(default=None, description="Optional city (owner-only in public views). Trimmed; empty is stored as null.")
    postal_code: Optional[StrictStr] = Field(default=None, description="Optional postal code (owner-only in public views). Trimmed; empty is stored as null.", alias="postalCode")
    operator_id: Optional[StrictStr] = Field(default=None, description="Optional owning Economic Operator id. Must be bound to your tenant workspace (403 otherwise). Empty/whitespace is treated as absent. Operator-scoped API keys may only use their own operator id (it is applied automatically when omitted).", alias="operatorId")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["gln", "name", "country", "activity", "streetAddress", "city", "postalCode", "operatorId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FacilityCreateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FacilityCreateRequest from a dict"""
        if obj is None:
            return None
