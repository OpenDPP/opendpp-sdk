...
class UntpEventCredential(BaseModel):
    """
    ...
    """ # noqa: E501
    at_context: Optional[Any] = Field(default=None, alias="@context")
    id: Optional[StrictStr] = Field(default=None, description="Credential id (e.g. `urn:uuid:...`). NOT used as the stored event id — the event primary key is always server-generated.")
    type: Optional[List[StrictStr]] = None
    issuer: Optional[StrictStr] = Field(default=None, description="Issuer DID. Unless a trusted x5c chain is embedded, the verification key is resolved by EXACT match of the DID's trailing `:`-segment against registered tenant subdomains/company names — e.g. `did:web:opendpp-node.eu:demo` resolves the workspace with subdomain `demo`. For operator-scoped API keys the issuer DID must ALSO contain the bound operator's registration id somewhere in the string (the issuer is checked in preference to `credentialSubject.responsibleOperatorDid`), e.g. `did:web:opendpp-node.eu:EU-DEFAULT-001:demo`. Stored verbatim as the event's `issuerDid`.")
    issuance_date: Optional[datetime] = Field(default=None, description="Fallback for the stored `eventTime` when `credentialSubject.eventTime` is absent.", alias="issuanceDate")
    credential_subject: UntpEventCredentialSubject = Field(alias="credentialSubject")
    proof: UntpEventProof
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@context", "id", "type", "issuer", "issuanceDate", "credentialSubject", "proof"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UntpEventCredential from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UntpEventCredential from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "credentialSubject": UntpEventCredentialSubject.from_dict(obj["credentialSubject"]) if obj.get("credentialSubject") is not None else None,
            "proof": UntpEventProof.from_dict(obj["proof"]) if obj.get("proof") is not None else None
        })
