...
PASSPORTGETTOOMANYREQUESTS_ANY_OF_SCHEMAS = ["ForwardedResolverRateLimitError", "GlobalRateLimitError"]

class PassportGetTooManyRequests(BaseModel):
    """
    ...
    # data type: GlobalRateLimitError
    anyof_schema_1_validator: Optional[GlobalRateLimitError] = None
    # data type: ForwardedResolverRateLimitError
    anyof_schema_2_validator: Optional[ForwardedResolverRateLimitError] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ForwardedResolverRateLimitError, GlobalRateLimitError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ForwardedResolverRateLimitError", "GlobalRateLimitError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PassportGetTooManyRequests.model_construct()
        error_messages = []
        # validate data type: GlobalRateLimitError
        if not isinstance(v, GlobalRateLimitError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GlobalRateLimitError`")
        else:
            return v

        # validate data type: ForwardedResolverRateLimitError
        if not isinstance(v, ForwardedResolverRateLimitError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ForwardedResolverRateLimitError`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PassportGetTooManyRequests with anyOf schemas: ForwardedResolverRateLimitError, GlobalRateLimitError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[GlobalRateLimitError] = None
        try:
            instance.actual_instance = GlobalRateLimitError.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ForwardedResolverRateLimitError] = None
        try:
            instance.actual_instance = ForwardedResolverRateLimitError.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PassportGetTooManyRequests with anyOf schemas: ForwardedResolverRateLimitError, GlobalRateLimitError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ForwardedResolverRateLimitError, GlobalRateLimitError]]:
