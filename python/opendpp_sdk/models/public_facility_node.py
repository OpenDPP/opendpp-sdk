...
class PublicFacilityNode(BaseModel):
    """
    ...
    id: StrictStr
    gln: Annotated[str, Field(strict=True)] = Field(description="GS1 GLN-13 with a valid modulo-10 check digit.")
    name: StrictStr
    activity: Optional[StrictStr] = Field(description="What the facility does in the chain, e.g. `cell assembly`.")
    country: StrictStr = Field(description="ISO 3166-1 alpha-2 country code.")
    street_address: Optional[StrictStr] = Field(default=None, description="Owner tier only — omitted from public and grant-tier responses.", alias="streetAddress")
    city: Optional[StrictStr] = Field(default=None, description="Owner tier only.")
    postal_code: Optional[StrictStr] = Field(default=None, description="Owner tier only.", alias="postalCode")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@type", "id", "gln", "name", "activity", "country", "streetAddress", "city", "postalCode"]

    @field_validator('at_type')
    def at_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Facility']):
            raise ValueError("must be one of enum values ('Facility')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PublicFacilityNode from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PublicFacilityNode from a dict"""
        if obj is None:
            return None
