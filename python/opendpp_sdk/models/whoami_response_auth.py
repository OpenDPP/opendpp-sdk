...
class WhoamiResponseAuth(BaseModel):
    """
    WhoamiResponseAuth
    """ # noqa: E501
    role: StrictStr = Field(description="The principal's role.")
    permissions: List[StrictStr] = Field(description="Effective permission strings, re-derived server-side from the role (never trusted from the token). May include wildcards like `operator:*`.")
    is_api_key_session: StrictBool = Field(description="`true` when authenticated with an API key, `false` for a session JWT.", alias="isApiKeySession")
    operator_id: Optional[StrictStr] = Field(description="The economic operator this credential is scoped to, or `null` for a workspace-wide key. A scoped key's writes and reads are restricted to this operator.", alias="operatorId")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["role", "permissions", "isApiKeySession", "operatorId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WhoamiResponseAuth from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WhoamiResponseAuth from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "isApiKeySession": obj.get("isApiKeySession"),
