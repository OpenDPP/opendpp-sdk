...
AASINGESTBADREQUEST_ANY_OF_SCHEMAS = ["AasIngestValidationError", "Error"]

class AasIngestBadRequest(BaseModel):
    """
    ...
    # data type: AasIngestValidationError
    anyof_schema_2_validator: Optional[AasIngestValidationError] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AasIngestValidationError, Error]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "AasIngestValidationError", "Error" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AasIngestBadRequest.model_construct()
        error_messages = []
        ...
        # validate data type: AasIngestValidationError
        if not isinstance(v, AasIngestValidationError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AasIngestValidationError`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in AasIngestBadRequest with anyOf schemas: AasIngestValidationError, Error. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[AasIngestValidationError] = None
        try:
            instance.actual_instance = AasIngestValidationError.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AasIngestBadRequest with anyOf schemas: AasIngestValidationError, Error. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AasIngestValidationError, Error]]:
