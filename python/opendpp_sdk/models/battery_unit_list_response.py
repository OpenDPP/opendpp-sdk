...
class BatteryUnitListResponse(BaseModel):
    """
    A page of the serialised battery units recorded under one passport, with the paging envelope.
    """ # noqa: E501
    success: StrictBool
    count: StrictInt = Field(description="Number of items returned in THIS page (≤ `limit`).")
    product_id: StrictStr = Field(description="The passport's caller-supplied product identifier (GTIN-14 / GRAI / SKU).", alias="productId")
    units: List[BatteryUnitRow] = Field(description="All units of the passport, `createdAt` DESC.")
    page: StrictInt = Field(description="1-based page number returned.")
    limit: StrictInt = Field(description="Effective page size (default 100, max 200).")
    total: StrictInt = Field(description="Total items matching across all pages.")
    total_pages: StrictInt = Field(description="Total number of pages (≥ 1).", alias="totalPages")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["success", "count", "productId", "units", "page", "limit", "total", "totalPages"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitListResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitListResponse from a dict"""
        if obj is None:
            return None
