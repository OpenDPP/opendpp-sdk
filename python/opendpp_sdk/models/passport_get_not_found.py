...
PASSPORTGETNOTFOUND_ANY_OF_SCHEMAS = ["Error", "ForwardedResolverError"]

class PassportGetNotFound(BaseModel):
    """
    ...
    # data type: ForwardedResolverError
    anyof_schema_2_validator: Optional[ForwardedResolverError] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, ForwardedResolverError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "ForwardedResolverError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PassportGetNotFound.model_construct()
        error_messages = []
        ...
        # validate data type: ForwardedResolverError
        if not isinstance(v, ForwardedResolverError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ForwardedResolverError`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PassportGetNotFound with anyOf schemas: Error, ForwardedResolverError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[ForwardedResolverError] = None
        try:
            instance.actual_instance = ForwardedResolverError.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PassportGetNotFound with anyOf schemas: Error, ForwardedResolverError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, ForwardedResolverError]]:
