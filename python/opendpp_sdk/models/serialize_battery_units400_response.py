...
SERIALIZEBATTERYUNITS400RESPONSE_ONE_OF_SCHEMAS = ["BatteryUnitSerialisationFailedError", "Error", "FastifyDefaultBadRequest"]

class SerializeBatteryUnits400Response(BaseModel):
    """
    SerializeBatteryUnits400Response
    """
    # data type: Error
    ...
    # data type: FastifyDefaultBadRequest
    oneof_schema_3_validator: Optional[FastifyDefaultBadRequest] = None
    actual_instance: Optional[Union[BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest]] = None
    one_of_schemas: Set[str] = { "BatteryUnitSerialisationFailedError", "Error", "FastifyDefaultBadRequest" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SerializeBatteryUnits400Response.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, BatteryUnitSerialisationFailedError):
            ...
            match += 1
        # validate data type: FastifyDefaultBadRequest
        if not isinstance(v, FastifyDefaultBadRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FastifyDefaultBadRequest`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SerializeBatteryUnits400Response with oneOf schemas: BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SerializeBatteryUnits400Response with oneOf schemas: BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into FastifyDefaultBadRequest
        try:
            instance.actual_instance = FastifyDefaultBadRequest.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SerializeBatteryUnits400Response with oneOf schemas: BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SerializeBatteryUnits400Response with oneOf schemas: BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BatteryUnitSerialisationFailedError, Error, FastifyDefaultBadRequest]]:
