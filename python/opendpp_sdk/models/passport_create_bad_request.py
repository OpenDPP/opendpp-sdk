...
PASSPORTCREATEBADREQUEST_ANY_OF_SCHEMAS = ["Error", "PassportCreateValidationError"]

class PassportCreateBadRequest(BaseModel):
    """
    ...
    # data type: PassportCreateValidationError
    anyof_schema_1_validator: Optional[PassportCreateValidationError] = None
    # data type: Error
    anyof_schema_2_validator: Optional[Error] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Error, PassportCreateValidationError]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Error", "PassportCreateValidationError" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PassportCreateBadRequest.model_construct()
        error_messages = []
        # validate data type: PassportCreateValidationError
        if not isinstance(v, PassportCreateValidationError):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PassportCreateValidationError`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PassportCreateBadRequest with anyOf schemas: Error, PassportCreateValidationError. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[PassportCreateValidationError] = None
        try:
            instance.actual_instance = PassportCreateValidationError.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PassportCreateBadRequest with anyOf schemas: Error, PassportCreateValidationError. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, PassportCreateValidationError]]:
