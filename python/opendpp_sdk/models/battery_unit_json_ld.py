...
class BatteryUnitJsonLd(BaseModel):
    """
    ...
    at_id: StrictStr = Field(description="The unit's GS1 Digital Link URI (same value as `digitalLinkUri`).", alias="@id")
    id: StrictStr
    serial_number: Annotated[str, Field(strict=True)] = Field(alias="serialNumber")
    ...
    successor_units: List[BatteryUnitLineageRef] = Field(description="Units repurposed/remanufactured from this one. **Always `[]` on `GET /api/v1/units/{id}`** (relation not loaded; see `repurposedFrom`).", alias="successorUnits")
    of_model: PublicPassportJsonLd = Field(description="The SKU/type-level passport this unit is an instance of. On this authenticated endpoint it is rendered in the **owner (privileged, unredacted) variant**: legitimate-interest-tier metadata keys and owner-only keys (e.g. `facilityDetails`) are NOT masked, unlike the anonymous public document this schema describes. The passport's own `@context` inline map always carries 9 fixed terms (DigitalProductPassport, economicOperator, manufacturingFacility, metadata, digitalSeal, signingPublicKey, status, archivedAt, retentionUntil) plus one dynamically generated term per metadata key.", alias="ofModel")
    current_state: Optional[BatteryUnitCurrentState] = Field(alias="currentState")
    dynamic_data: Annotated[List[BatteryUnitDynamicDataEvent], Field(max_length=500)] = Field(description="Full telemetry history, newest first by `recordedAt`, capped at the 500 most recent events.", alias="dynamicData")
    created_at: datetime = Field(alias="createdAt")
    updated_at: datetime = Field(alias="updatedAt")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["@context", "@type", "@id", "id", "serialNumber", "digitalLinkUri", "status", "manufacturedAt", "repurposedFrom", "successorUnits", "ofModel", "currentState", "dynamicData", "createdAt", "updatedAt"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BatteryUnitJsonLd from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BatteryUnitJsonLd from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "dynamicData": [BatteryUnitDynamicDataEvent.from_dict(_item) for _item in obj["dynamicData"]] if obj.get("dynamicData") is not None else None,
