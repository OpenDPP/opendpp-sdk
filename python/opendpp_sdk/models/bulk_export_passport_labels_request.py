...
class BulkExportPassportLabelsRequest(BaseModel):
    """
    BulkExportPassportLabelsRequest
    """ # noqa: E501
    ids: Annotated[List[StrictStr], Field(min_length=1, max_length=200)] = Field(description="Passport UUIDs and/or `productId`s (GTIN-14/GRAI/SKU), 1–200 items. Each is resolved tenant-scoped; unresolvable / not-owned ids are skipped and listed in `manifest.json`.")
    format: Optional[StrictStr] = Field(default='png', description="Image format for every label in the ZIP.")
    size: Optional[Annotated[int, Field(le=2048, strict=True, ge=128)]] = Field(default=1024, description="Rendered width in px; clamped to 128–2048. A PNG is never rendered below one pixel per module, so an unusually dense symbol comes back slightly wider than requested (at most 185 px).")
    ecl: Optional[StrictStr] = Field(default='Q', description="QR error-correction level (GS1 product-label guidance: `Q`).")
    hri: Optional[StrictBool] = Field(default=False, description="Print the Human-Readable Interpretation beneath each symbol (EN 18220 5.7.2 — what the carrier encodes): the GS1 element string for a GS1 Digital Link, the link URL itself for an Identification Link. Requires `format: \"svg\"`.")
    x_dimension_mm: Optional[Union[Annotated[float, Field(le=2, strict=True, ge=0.396)], Annotated[int, Field(le=2, strict=True, ge=1)]]] = Field(default=None, description="Width of ONE module in millimetres for every symbol in the ZIP (EN 18220 5.5.2), range 0.396–2 mm. Sizes the SVG root in mm instead of pixels; `format: \"svg\"` only, and not combinable with `hri`.", alias="xDimensionMm")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["ids", "format", "size", "ecl", "hri", "xDimensionMm"]
    ...
    def format_validate_enum(cls, value):
        ...
        if value not in set(['png', 'svg']):
            raise ValueError("must be one of enum values ('png', 'svg')")
        return value

    @field_validator('ecl')
    def ecl_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['M', 'Q', 'H']):
            raise ValueError("must be one of enum values ('M', 'Q', 'H')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BulkExportPassportLabelsRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BulkExportPassportLabelsRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "size": obj.get("size") if obj.get("size") is not None else 1024,
            "ecl": obj.get("ecl") if obj.get("ecl") is not None else 'Q',
            "hri": obj.get("hri") if obj.get("hri") is not None else False,
