 ...
 */
export const listPassportHistory = <ThrowOnError extends boolean = false>(options: Options<ListPassportHistoryData, ThrowOnError>): RequestResult<ListPassportHistoryResponses, ListPassportHistoryErrors, ThrowOnError> => (options.client ?? client).get<ListPassportHistoryResponses, ListPassportHistoryErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 * **Who may read this.** Two authorised actors, and the anonymous tier is refused — EN 18221:2026 §4.2 makes an archived version retrievable by authenticated and authorised actors ONLY. (1) The **owning or operator-bound tenant**, via an `op_dpp_token_…` API key: the version is unmasked, exactly as that tenant's live read is. (2) The holder of a **legitimate-interest or authority grant** covering this passport (`dpp_li_…` / `dpp_auth_…` as a Bearer token or `?grant=`): the version is masked to the grant tier — the Battery Reg. Annex XIII(2)-(4) restricted keys are readable, the owner-only keys are not — so a version discloses exactly what the live read discloses to the same caller, and no more. A grant-unlocked response carries `Cache-Control: private, no-store` and `Referrer-Policy: no-referrer`. Attribution (`changedBy`, `changeReason`) is owner-only and reads `null` for a grant holder: it names the staff who edited a row, not regulated product data. A DRAFT is never revealed to a grant. Every read is recorded in the passport's access audit log.
 */
export const getPassportVersionAtDate = <ThrowOnError extends boolean = false>(options: Options<GetPassportVersionAtDateData, ThrowOnError>): RequestResult<GetPassportVersionAtDateResponses, GetPassportVersionAtDateErrors, ThrowOnError> => (options.client ?? client).get<GetPassportVersionAtDateResponses, GetPassportVersionAtDateErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 * **Who may read this.** Two authorised actors, and the anonymous tier is refused — EN 18221:2026 §4.2 makes an archived version retrievable by authenticated and authorised actors ONLY. (1) The **owning or operator-bound tenant**, via an `op_dpp_token_…` API key: the version is unmasked, exactly as that tenant's live read is. (2) The holder of a **legitimate-interest or authority grant** covering this passport (`dpp_li_…` / `dpp_auth_…` as a Bearer token or `?grant=`): the version is masked to the grant tier — the Battery Reg. Annex XIII(2)-(4) restricted keys are readable, the owner-only keys are not — so a version discloses exactly what the live read discloses to the same caller, and no more. A grant-unlocked response carries `Cache-Control: private, no-store` and `Referrer-Policy: no-referrer`. Attribution (`changedBy`, `changeReason`) is owner-only and reads `null` for a grant holder: it names the staff who edited a row, not regulated product data. A DRAFT is never revealed to a grant. Every read is recorded in the passport's access audit log.
 */
export const getPassportVersion = <ThrowOnError extends boolean = false>(options: Options<GetPassportVersionData, ThrowOnError>): RequestResult<GetPassportVersionResponses, GetPassportVersionErrors, ThrowOnError> => (options.client ?? client).get<GetPassportVersionResponses, GetPassportVersionErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 */
export const getDppVocabulary = <ThrowOnError extends boolean = false>(options?: Options<GetDppVocabularyData, ThrowOnError>): RequestResult<GetDppVocabularyResponses, GetDppVocabularyErrors, ThrowOnError> => (options?.client ?? client).get<GetDppVocabularyResponses, GetDppVocabularyErrors, ThrowOnError>({ url: '/ns/dpp', ...options });
